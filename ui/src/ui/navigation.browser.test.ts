...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/opencraft/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/opencraft");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/opencraft/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCRAFT_CONTROL_UI_BASE_PATH__ = "/opencraft";
    const app = mountApp("/opencraft/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/opencraft");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/opencraft/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("opencraft.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/opencraft" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;
    ...
    expect(JSON.parse(localStorage.getItem("opencraft.control.settings.v1") ?? "{}")).toMatchObject(
      {
        gatewayUrl: "wss://gateway.example/opencraft",
      },
    );
    expect(JSON.parse(localStorage.getItem("opencraft.control.settings.v1") ?? "{}").token).toBe(
      undefined,
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/opencraft";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/opencraft");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/opencraft#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/opencraft");
    expect(app.settings.token).toBe("");
