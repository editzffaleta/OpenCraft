...
Deploy OpenCraft on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/editzffaleta/OpenCraft)
...
repository configures everything needed to run OpenCraft:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENCRAFT_STATE_DIR
        value: /data/.opencraft
      - key: OPENCRAFT_WORKSPACE_DIR
...
The web dashboard is available at `https://<your-service>.onrender.com/opencraft`.
...
If you use the original OpenCraft repository, Render will not auto-deploy your OpenCraft. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenCraft, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenCraft host.
