...
Deploy OpenCraft on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/opencraft` — Control UI
...
- `OPENCRAFT_STATE_DIR=/data/.opencraft` (recommended)
- `OPENCRAFT_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENCRAFT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenCraft state + workspace so you can migrate to another host without losing config or memory.
