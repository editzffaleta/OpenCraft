...
Run the OpenCraft gateway in a **rootless** Podman container. Uses the same image as Docker (build from the repo [Dockerfile](https://github.com/editzffaleta/OpenCraft/blob/main/Dockerfile)).
...
This also creates a minimal `~opencraft/.editzffaleta/OpenCraft.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
- **Config:** Only `opencraft` and root can access `/home/opencraft/.opencraft`. To edit config: use the Control UI once the gateway is running, or `sudo -u opencraft $EDITOR /home/opencraft/.editzffaleta/OpenCraft.json`.

## Environment and config

- **Token:** Stored in `~opencraft/.opencraft/.env` as `OPENCLAW_GATEWAY_TOKEN`. `setup-podman.sh` and `run-opencraft-podman.sh` generate it if missing (uses `openssl`, `python3`, or `od`).
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~opencraft/.editzffaleta/OpenCraft.json` exists and sets `gateway.mode="local"`. `setup-podman.sh` creates this file if missing.
