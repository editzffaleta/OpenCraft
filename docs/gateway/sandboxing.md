...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`opencraft-sandbox-browser`) instead of the global `bridge` network.
...
Default Docker image: `opencraft-sandbox:bookworm-slim`
...
`opencraft-sandbox-common:bookworm-slim`.
