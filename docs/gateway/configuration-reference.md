...
- Legacy single-account Baileys auth dir is migrated by `opencraft doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `opencraft doctor` warns when this is missing or invalid.
...
- OpenCraft additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `opencraft plugins install @opencraft/mattermost`.
...
- `commands.callbackUrl` must resolve to the OpenCraft gateway endpoint and be reachable from the Mattermost server.
...
OpenCraft spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `opencraft channels add` (or channel onboarding) while still on a single-account top-level channel config, OpenCraft moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `opencraft doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
- `config: true` enables `/config` (reads/writes `opencraft.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenCraft auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenCraft assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before OpenCraft aborts it. Default: `900`.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that OpenCraft materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.opencraft/sandboxes`
...
In `remote` mode, host-local edits made outside OpenCraft are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `opencraft.json`.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `OPENCRAFT_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `OPENCRAFT_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `OPENCRAFT_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
...
For `type: "acp"` entries, OpenCraft resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, OpenCraft starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, OpenCraft treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
- Files are materialized into the child workspace at `.opencraft/attachments/<uuid>/` with a `.manifest.json`.
...
OpenCraft uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.opencraft/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENCRAFT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), OpenCraft forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `opencraft onboard --auth-choice opencode-zen` or `opencraft onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `opencraft onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `opencraft onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `opencraft onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `opencraft onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `opencraft onboard --auth-choice minimax-api`.
...
- Loaded from `~/.opencraft/extensions`, `<workspace>/.opencraft/extensions`, plus `plugins.load.paths`.
- Discovery accepts native OpenCraft plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.installs`: CLI-managed install metadata used by `opencraft plugins update`.
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENCRAFT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `OPENCRAFT_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `OPENCRAFT_APNS_RELAY_BASE_URL` / `OPENCRAFT_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `OPENCRAFT_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
OPENCRAFT_CONFIG_PATH=~/.opencraft/a.json \
OPENCRAFT_STATE_DIR=~/.opencraft-a \
opencraft gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.opencraft-dev` + port `19001`), `--profile <name>` (uses `~/.opencraft-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-opencraft-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENCRAFT_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENCRAFT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__opencraft__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__opencraft__/a2ui/`
...
- Also serves A2UI at `/__opencraft__/a2ui/`.
...
- Hostname defaults to `opencraft`. Override with `OPENCRAFT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.opencraft/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `opencraft dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.opencraft/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `opencraft.json` credential paths.
...
- Legacy OAuth imports from `~/.opencraft/credentials/oauth.json`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one OpenCraft.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `OPENCRAFT_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `opencraft doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `opencraft.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
