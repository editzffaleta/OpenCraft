...
- `opencraft gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `opencraft doctor` reports no blocking config/service issues.
- `opencraft channels status --probe` shows connected/ready channels.
...
opencraft browser start --browser-profile opencraft
