...
- Turn delivery on:
  ...
  - or start with `opencraft tui --deliver`
...
- Local shell commands receive `OPENCRAFT_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCRAFT_THEME=light` before launching `opencraft tui`.
- To force the original dark palette instead, set `OPENCRAFT_THEME=dark`.
...
- Check the Gateway logs: `opencraft logs --follow`.
- Confirm the agent can run: `opencraft status` and `opencraft models status`.
...
- No agents in picker: check `opencraft agents list` and your routing config.
