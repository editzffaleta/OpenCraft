...
If the page fails to load, start the Gateway first: `opencraft gateway`.
...
you revoke it with `opencraft devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.editzffaleta/OpenCraft.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop opencraft`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenCraft
...
OpenCraft **blocks** Control UI connections without device identity.
