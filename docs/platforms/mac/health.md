...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`opencraft status`, `opencraft status --deep`, `opencraft health --json`) and tail `/tmp/editzffaleta/OpenCraft-*.log` for `web-heartbeat` / `web-reconnect`.
