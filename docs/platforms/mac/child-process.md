...
- The app installs a per‑user LaunchAgent labeled `ai.opencraft.gateway`
  (or `ai.opencraft.<profile>` when using `--profile`/`OPENCRAFT_PROFILE`; legacy `com.opencraft.*` is supported).
