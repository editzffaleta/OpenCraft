...
summary: OpenCraft Gateway 网关 CLI（`opencraft gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 OpenCraft 的 WebSocket 服务器（渠道、节点、会话、hooks）。

本页中的子命令位于 `opencraft gateway …` 下。
...
`gateway discover` 扫描 Gateway 网关信标（`_opencraft-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`opencraft.internal.`）并设置分割 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
