...
5. **跟踪最新日志**
   ...
   ```bash
   tail -f "$(ls -t /tmp/opencraft/opencraft-*.log | head -1)"
   ```
...
- 令牌与 `gateway.auth.token`（或 `OPENCRAFT_GATEWAY_TOKEN`）的值相同，UI 在首次加载后会存储它。
...
使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.opencraft/skills/<name>/SKILL.md`（或通过 `~/.opencraft/opencraft.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.opencraft/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.opencraft/opencraft.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.opencraft/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenCraft 将其视为 `<workspace>/skills`。
...
OpenCraft 从 `$OPENCRAFT_CONFIG_PATH`（默认：`~/.opencraft/opencraft.json`）读取可选的 **JSON5** 配置：
...
非 local loopback 绑定**需要认证**。配置 `gateway.auth.mode` + `gateway.auth.token`（或使用 `OPENCRAFT_GATEWAY_TOKEN`）。
...
- 从备份恢复（git 或复制的 `~/.opencraft/opencraft.json`）。
...
- 编辑 `~/.opencraft/opencraft.json` 中的 `agents.defaults.model`
...
- 在 Gateway 网关主机上设置 `gateway.auth.token`（或 `OPENCRAFT_GATEWAY_TOKEN`）。
