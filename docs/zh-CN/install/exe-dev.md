...
summary: 在 exe.dev 上运行 OpenCraft Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：让 OpenCraft Gateway 网关运行在 exe.dev VM 上，并且可通过你的笔记本电脑访问：`https://<vm-name>.exe.xyz`
...
Shelley 是 [exe.dev](https://exe.dev) 的智能体，可以使用我们的提示词立即安装 OpenCraft。
使用的提示词如下：
...
提示：请让这个 VM 保持**有状态**。OpenCraft 会将状态存储在 `~/.opencraft/` 和 `~/.opencraft/workspace/` 下。
...
## 4）设置 nginx，将 OpenCraft 代理到端口 8000
...
## 5）访问 OpenCraft 并授予权限

访问 `https://<vm-name>.exe.xyz/`（请查看新手引导输出中的控制 UI）。如果提示进行身份验证，请粘贴 VM 上的
`gateway.auth.token` 中的令牌（可通过 `opencraft config get gateway.auth.token` 获取，或使用
`opencraft doctor --generate-gateway-token` 生成）。使用 `opencraft devices list` 和
`opencraft devices approve <requestId>` 批准设备。如果拿不准，请在浏览器中使用 Shelley！
