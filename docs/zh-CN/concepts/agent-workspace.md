...
这与存储配置、凭证和会话的 `~/.opencraft/` 是分开的。

**重要：**工作区是**默认 cwd**，而不是硬性沙箱。工具会相对于工作区解析相对路径，但除非启用沙箱隔离，否则绝对路径仍然可以访问主机上的其他位置。如果你需要隔离，请使用 [`agents.defaults.sandbox`](/gateway/sandboxing)（和/或按智能体配置的沙箱）。启用沙箱隔离后，如果 `workspaceAccess` 不是 `"rw"`，工具会在 `~/.opencraft/sandboxes` 下的沙箱工作区中运行，而不是在你的主机工作区中运行。
...
`opencraft onboard`、`opencraft configure` 或 `opencraft setup` 会在工作区缺失时创建工作区并植入引导文件。
...
较旧的安装可能创建过 `~/opencraft`。保留多个工作区目录可能会导致令人困惑的凭证或状态漂移，因为同一时间只有一个工作区处于活动状态。

**建议：**只保留一个活动工作区。如果你不再使用额外的文件夹，请将其归档或移到废纸篓（例如 `trash ~/opencraft`）。
如果你有意保留多个工作区，请确保
`agents.defaults.workspace` 指向当前活动的那个。

当 `opencraft doctor` 检测到额外的工作区目录时，会发出警告。

## 工作区文件映射（每个文件的含义）

以下是 OpenCraft 在工作区内预期的标准文件：
...
如果任何引导文件缺失，OpenCraft 会在会话中注入一个“缺失文件”标记并继续执行。注入时，大型引导文件会被截断；可使用 `agents.defaults.bootstrapMaxChars`（默认：20000）和 `agents.defaults.bootstrapTotalMaxChars`（默认：150000）调整限制。
`opencraft setup` 可以重新创建缺失的默认文件，而不会覆盖现有文件。
...
如果你必须存储敏感引用，请使用占位符，并将真实密钥保存在其他地方（密码管理器、环境变量或 `~/.opencraft/`）。
