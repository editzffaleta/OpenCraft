...
OpenCraft 运行一个源自 **pi-mono** 的嵌入式智能体运行时。
...
在 `agents.defaults.workspace` 内，OpenCraft 期望以下用户可编辑的文件：
...
在新会话的第一轮，OpenCraft 将这些文件的内容直接注入智能体上下文。

空文件会被跳过。大文件会被修剪和截断并添加标记，以保持提示词精简（阅读文件获取完整内容）。

如果文件缺失，OpenCraft 会注入一行"文件缺失"标记（`opencraft setup` 将创建安全的默认模板）。
...
OpenCraft 从三个位置加载 Skills（名称冲突时工作区优先）：

- 内置（随安装包提供）
- 托管/本地：`~/.opencraft/skills`
...
OpenCraft 复用 pi-mono 代码库的部分内容（模型/工具），但**会话管理、设备发现和工具连接由 OpenCraft 负责**。
...
- 如果省略提供商，OpenCraft 将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
