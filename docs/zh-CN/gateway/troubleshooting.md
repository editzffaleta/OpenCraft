...
read_when:
  - 调查运行时问题或故障
summary: OpenCraft 常见故障的快速故障排除指南
title: 故障排除
x-i18n:
...
| `opencraft gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `opencraft channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `opencraft gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `opencraft logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `opencraft status --all`（它会隐藏令牌）。如果你粘贴 `opencraft status`，考虑先设置 `OPENCRAFT_SHOW_SECRETS=0`（令牌预览）。
...
- 或直接设置：
  ```bash
  opencraft config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  opencraft config set gateway.mode remote
  opencraft config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `opencraft setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.opencraft/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `opencraft doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `opencraft sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `opencraft gateway` 和服务。
...
**如果 `opencraft gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `OPENCRAFT_STATE_DIR` 重新运行 `opencraft gateway install --force`。

**如果 `opencraft gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `opencraft doctor` 更新它（或 `opencraft gateway install --force` 完全重写）。
...
症状：`pwd` 或文件工具显示 `~/.opencraft/sandboxes/...` 即使你
期望的是主机工作区。
...
OpenCraft 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `opencraft models list` 或
  `opencraft models scan` 并选择一个支持的模型。
...
```bash
opencraft logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.opencraft/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
opencraft logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
opencraft channels logout
trash "${OPENCRAFT_STATE_DIR:-$HOME/.opencraft}/credentials" # 如果 logout 无法完全清除所有内容
opencraft channels login --verbose       # 重新扫描二维码
```
...
当配置包含未知键、格式错误的值或无效类型时，OpenCraft 现在拒绝启动。
这是为了安全而故意设计的。
...
- `opencraft doctor` 报告每个无效条目。
- `opencraft doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `opencraft logs`、`opencraft health`、`opencraft status`、`opencraft gateway status` 和 `opencraft gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/opencraft/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `opencraft models status`（CLI）。
...
5. 运行 `opencraft channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。OpenCraft 在当前 `main` 中清理/规范化工具
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.opencraft.* 仍然有效）
```
...
**修复 3：检查 CLI 安装**
确保全局 `opencraft` CLI 已安装且与应用版本匹配：
...
opencraft gateway --verbose
opencraft channels login --verbose
```
...
trash "${OPENCRAFT_STATE_DIR:-$HOME/.opencraft}"
opencraft channels login         # 重新配对 WhatsApp
opencraft gateway restart           # 或：opencraft gateway
```
...
1. 首先检查日志：`/tmp/opencraft/`（默认：`opencraft-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - OpenCraft 版本
   - 相关日志片段
