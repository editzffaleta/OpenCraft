...
OpenCraft 会从 `~/.editzffaleta/OpenCraft.json` 读取可选的 <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> 配置。
...
<Tabs>
  ...
  <Tab title="Direct edit">
    直接编辑 `~/.editzffaleta/OpenCraft.json`。Gateway 网关会监视该文件并自动应用更改（参见[热重载](#config-hot-reload)）。
  </Tab>
</Tabs>
...
Gateway 网关会监视 `~/.editzffaleta/OpenCraft.json` 并自动应用更改 —— 对于大多数设置，无需手动重启。
