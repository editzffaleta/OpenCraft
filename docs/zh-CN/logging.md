...
- OpenCraft 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `opencraft plugins enable diagnostics-otel` 启用插件。
...
- `opencraft.tokens`（计数器，属性：`opencraft.token`、`opencraft.channel`、`opencraft.provider`、`opencraft.model`）
- `opencraft.cost.usd`（计数器，属性：`opencraft.channel`、`opencraft.provider`、`opencraft.model`）
- `opencraft.run.duration_ms`（直方图，属性：`opencraft.channel`、`opencraft.provider`、`opencraft.model`）
- `opencraft.context.tokens`（直方图，属性：`opencraft.context`、`opencraft.channel`、`opencraft.provider`、`opencraft.model`）

消息流：

- `opencraft.webhook.received`（计数器，属性：`opencraft.channel`、`opencraft.webhook`）
- `opencraft.webhook.error`（计数器，属性：`opencraft.channel`、`opencraft.webhook`）
- `opencraft.webhook.duration_ms`（直方图，属性：`opencraft.channel`、`opencraft.webhook`）
- `opencraft.message.queued`（计数器，属性：`opencraft.channel`、`opencraft.source`）
- `opencraft.message.processed`（计数器，属性：`opencraft.channel`、`opencraft.outcome`）
- `opencraft.message.duration_ms`（直方图，属性：`opencraft.channel`、`opencraft.outcome`）

队列 + 会话：

- `opencraft.queue.lane.enqueue`（计数器，属性：`opencraft.lane`）
- `opencraft.queue.lane.dequeue`（计数器，属性：`opencraft.lane`）
- `opencraft.queue.depth`（直方图，属性：`opencraft.lane` 或 `opencraft.channel=heartbeat`）
- `opencraft.queue.wait_ms`（直方图，属性：`opencraft.lane`）
- `opencraft.session.state`（计数器，属性：`opencraft.state`、`opencraft.reason`）
- `opencraft.session.stuck`（计数器，属性：`opencraft.state`）
- `opencraft.session.stuck_age_ms`（直方图，属性：`opencraft.state`）
- `opencraft.run.attempt`（计数器，属性：`opencraft.attempt`）
...
- `opencraft.webhook.error`
  - `opencraft.channel`、`opencraft.webhook`、`opencraft.chatId`、`opencraft.error`
- `opencraft.message.processed`
  - `opencraft.channel`、`opencraft.outcome`、`opencraft.chatId`、`opencraft.messageId`、`opencraft.sessionKey`、`opencraft.sessionId`、`opencraft.reason`
- `opencraft.session.stuck`
  - `opencraft.state`、`opencraft.ageMs`、`opencraft.queueDepth`、`opencraft.sessionKey`、`opencraft.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opencraft doctor`。
