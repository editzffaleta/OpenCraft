...
Capabilities are the public plugin model. Every native OpenCraft plugin
...
OpenCraft classifies every loaded plugin into a shape based on its actual
...
Use `opencraft plugins inspect <id>` to see a plugin's shape and capability
...
normal chat actions. OpenCraft keeps one shared `message` tool in core, and
...
A company plugin should feel cohesive from the outside. If OpenCraft has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { OpenCraftPluginDefinition } from "opencraft/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "opencraft/plugin-sdk";

const plugin: OpenCraftPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
OpenCraft already treats image/audio/video understanding as one shared
...
If OpenCraft adds a new domain later, such as video generation, use the same
...
Native OpenCraft plugins can register capabilities and surfaces:
...
OpenCraft exports capabilities, not implementation convenience.
...
Use `opencraft plugins inspect <id>` for deep plugin introspection. This is the
canonical command for understanding a plugin's shape and registration behavior.

```bash
opencraft plugins inspect openai
opencraft plugins inspect openai --json
```
...
- If OpenCraft adds a new capability such as video generation later, define the
...
- OpenCraft only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "OpenCraft plugin runtime helpers",
    count: 5,
  },
});
```
...
- `opencraft/plugin-sdk/core` for the smallest generic plugin-facing contract.
- Domain subpaths such as `opencraft/plugin-sdk/channel-config-helpers`,
  `opencraft/plugin-sdk/channel-config-schema`,
  `opencraft/plugin-sdk/channel-policy`,
  `opencraft/plugin-sdk/lazy-runtime`,
  `opencraft/plugin-sdk/reply-history`,
  `opencraft/plugin-sdk/routing`,
  `opencraft/plugin-sdk/runtime-store`, and
  `opencraft/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `opencraft/plugin-sdk/compat` remains as a legacy migration surface for older
...
- Bundled extension internals remain private. External plugins should use only
  `opencraft/plugin-sdk/*` subpaths. OpenCraft core/test code may use the repo
