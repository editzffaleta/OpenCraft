...
read_when:
  - You want to publish a third-party OpenCraft plugin
...
This page tracks high-quality **community-maintained plugins** for OpenCraft.

We accept PRs that add community plugins here when they meet the quality bar.

## Required for listing

- Plugin package is published on npmjs (installable via `opencraft plugins install <npm-spec>`).
...
- **WeChat** — Connect OpenCraft to WeChat personal accounts via WeChatPadPro (iPad protocol). Supports text, image, and file exchange with keyword-triggered conversations.
