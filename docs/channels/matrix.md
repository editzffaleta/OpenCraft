...
Matrix is an open, decentralized messaging protocol. OpenCraft connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `opencraft plugins install @opencraft/matrix`
   - From a local checkout: `opencraft plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenCraft calls the same
     login endpoint, stores the access token in `~/.opencraft/credentials/matrix/credentials.json`,
...
- On first connection, OpenCraft requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenCraft logs a warning.
...
- On startup, OpenCraft resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
