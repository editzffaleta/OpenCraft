...
Config helpers for non-interactive edits in `opencraft.json`: get/set/unset/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `opencraft configure`).
...
opencraft config set secrets.providers.vaultfile --provider-source file --provider-path /etc/opencraft/secrets.json --provider-mode json
...
`opencraft config set` supports four assignment styles:

1. Value mode: `opencraft config set <path> <value>`
2. SecretRef builder mode:

```bash
opencraft config set channels.discord.token \
...
```bash
opencraft config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/opencraft-vault \
  --provider-arg read \
...
```bash
opencraft config set --batch-json '[
  {
...
opencraft config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/opencraft/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `opencraft.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.opencraft/opencraft.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
