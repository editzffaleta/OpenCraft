---
summary: "CLI reference for `opencraft agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
If you omit `accountId` (`--bind <channel>`), OpenCraft resolves it from channel defaults and plugin setup hooks when available.

### Binding scope behavior

- A binding without `accountId` matches the channel default account only.
- `accountId: "*"` is the channel-wide fallback (all accounts) and is less specific than an explicit account binding.
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, OpenCraft upgrades that existing binding in place instead of adding a duplicate.
...
- Example path: `~/.opencraft/workspace/IDENTITY.md`
