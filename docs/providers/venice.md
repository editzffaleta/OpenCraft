---
summary: "Use Venice AI privacy-focused models in OpenCraft"
read_when:
  - You want privacy-focused inference in OpenCraft
...
```bash
opencraft agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, OpenCraft shows all available Venice models. Pick based on your needs:
...
OpenCraft automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opencraft agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
opencraft agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
opencraft agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
opencraft agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
opencraft agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `opencraft models list` to see currently available models. Some models may be temporarily offline.
