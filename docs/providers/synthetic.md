---
summary: "Usar a API compatível com Anthropic do Synthetic no OpenCraft"
read_when:
  - Você quer usar o Synthetic como provedor de modelo
  - Você precisa de configuração de chave de API ou URL base do Synthetic
title: "Synthetic"
---

# Synthetic

O Synthetic expõe endpoints compatíveis com Anthropic. O OpenCraft o registra como
provedor `synthetic` e usa a API de Mensagens Anthropic.

## Configuração rápida

1. Defina `SYNTHETIC_API_KEY` (ou execute o wizard abaixo).
2. Execute o onboarding:

```bash
opencraft onboard --auth-choice synthetic-api-key
```
...
Nota: O cliente Anthropic do OpenCraft adiciona `/v1` à URL base, então use
`https://api.synthetic.new/anthropic` (não `/anthropic/v1`). Se o Synthetic mudar
sua URL base, sobrescreva `models.providers.synthetic.baseUrl`.

## Catálogo de modelos

Todos os modelos abaixo usam custo `0` (entrada/saída/cache).

| ID do Modelo                                           | Janela de contexto | Max tokens | Raciocínio | Entrada      |
| ------------------------------------------------------ | ------------------ | ---------- | ---------- | ------------ |
...
| `hf:Qwen/Qwen3-VL-235B-A22B-Instruct`                  | 250000             | 8192       | false      | texto + img  |
...
- Refs de modelo usam `synthetic/<modelId>`.
- Se você habilitar uma allowlist de modelos (`agents.defaults.models`), adicione cada modelo que
  planeja usar.
- Veja [Provedores de modelo](/concepts/model-providers) para regras de provedor.
