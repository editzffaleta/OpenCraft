...
CLI onboarding is the **recommended** way to set up OpenCraft on macOS,
...
`opencraft dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
2. **Workspace** — Location for agent files (default `~/.opencraft/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `opencraft doctor` first.
</Note>
...
Use `opencraft agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.opencraft/workspace-<agentId>`.
...
- CLI command reference: [`opencraft onboard`](/cli/onboard)
