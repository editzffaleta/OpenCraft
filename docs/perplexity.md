...
OpenCraft supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, OpenCraft also supports legacy Perplexity Sonar/OpenRouter setups.
...
`~/.opencraft/.env` (or your service environment). See [Env vars](/help/faq#how-does-opencraft-load-environment-variables).
