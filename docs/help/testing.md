...
OpenCraft has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - OpenCraft uses Vitest `vmForks` on Node 22, 23, and 24 for faster unit shards.
  - On Node 25+, OpenCraft automatically falls back to regular `forks` until the repo is re-validated there.
  - Override manually with `OPENCRAFT_TEST_VM_FORKS=0` (force `forks`) or `OPENCRAFT_TEST_VM_FORKS=1` (force `vmForks`).
...
- Useful overrides:
  - `OPENCRAFT_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `OPENCRAFT_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises OpenCraft's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `OPENCRAFT_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `OPENCRAFT_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `OPENCRAFT_LIVE_TEST=1`)
- Scope:
...
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `OPENCRAFT_LIVE_*_KEY`; tests retry on rate limit responses.
...
- Optional target overrides:
  - `OPENCRAFT_ANDROID_NODE_ID` or `OPENCRAFT_ANDROID_NODE_NAME`.
...
- How to enable:
  - `pnpm test:live` (or `OPENCRAFT_LIVE_TEST=1` if invoking Vitest directly)
- Set `OPENCRAFT_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `OPENCRAFT_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.5, Grok 4)
  - `OPENCRAFT_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `OPENCRAFT_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
- How to select providers:
  - `OPENCRAFT_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `OPENCRAFT_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what “@opencraft” actually does)
...
- How to select models:
  ...
  - `OPENCRAFT_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `OPENCRAFT_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
- How to select providers (avoid “OpenRouter everything”):
  - `OPENCRAFT_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Token sources (pick one):
  - Profile: `OPENCRAFT_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `OPENCRAFT_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
...
- Overrides (optional):
  ...
  - `OPENCRAFT_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `OPENCRAFT_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `OPENCRAFT_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `OPENCRAFT_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
- `OPENCRAFT_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).
...
- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `OPENCRAFT_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: OpenCraft calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: OpenCraft shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
Include at least one image-capable model in `OPENCRAFT_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `opencraft models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `opencraft models list` / model selection.

- Profile store: `~/.opencraft/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.opencraft/opencraft.json` (or `OPENCRAFT_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `OPENCRAFT_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `OPENCRAFT_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `OPENCRAFT_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `OPENCRAFT_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
`OPENCRAFT_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
...
- `OPENCRAFT_CONFIG_DIR=...` (default: `~/.opencraft`) mounted to `/home/node/.opencraft`
- `OPENCRAFT_WORKSPACE_DIR=...` (default: `~/.opencraft/workspace`) mounted to `/home/node/.opencraft/workspace`
- `OPENCRAFT_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
...
- `OPENCRAFT_LIVE_GATEWAY_MODELS=...` / `OPENCRAFT_LIVE_MODELS=...` to narrow the run
- `OPENCRAFT_LIVE_GATEWAY_PROVIDERS=...` / `OPENCRAFT_LIVE_PROVIDERS=...` to filter providers in-container
- `OPENCRAFT_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
