#!/usr/bin/env bash
# Rootless OpenCraft in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./setup-podman.sh
# Then:
#   ./scripts/run-opencraft-podman.sh launch           # Start gateway
#   ./scripts/run-opencraft-podman.sh launch setup      # Onboarding wizard
#
# As the opencraft user (no repo needed):
...
CONFIG_JSON="$CONFIG_DIR/opencraft.json"
