...
CHECK_LATEST_REF=1
JSON_OUTPUT=0
GUEST_OPENCLAW_BIN="/opt/homebrew/bin/opencraft"
GUEST_OPENCLAW_ENTRY="/opt/homebrew/lib/node_modules/editzffaleta/OpenCraft.mjs"
...
send -- "cat >/tmp/opencraft-prl.sh <<'__OPENCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__OPENCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/opencraft-prl.sh; rc=\$?; rm -f /tmp/opencraft-prl.sh; printf '__OPENCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
