...
IMAGE_NAME="opencraft-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  OPENCRAFT_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  OPENCRAFT_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export OPENCRAFT_ENTRY

home_dir=$(mktemp -d "/tmp/opencraft-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/opencraft.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/opencraft.plugin.json" "$id"
...
mkdir -p "$HOME/.opencraft/extensions/demo-plugin"

cat > "$HOME/.opencraft/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.opencraft/extensions/demo-plugin/opencraft.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/opencraft-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@opencraft/demo-plugin-tgz",
  "version": "0.0.1",
  "opencraft": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/opencraft.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$OPENCRAFT_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/opencraft-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@opencraft/demo-plugin-dir",
  "version": "0.0.1",
  "opencraft": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/opencraft.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$OPENCRAFT_ENTRY" plugins install "$dir_plugin"
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/opencraft-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@opencraft/demo-plugin-file",
  "version": "0.0.1",
  "opencraft": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/opencraft.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$OPENCRAFT_ENTRY" plugins install "file:$file_pack_dir/package"
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "opencraft/fixture-marketplace"
    }
  }
}
JSON

node "$OPENCRAFT_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$OPENCRAFT_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$OPENCRAFT_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".opencraft", "opencraft.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$OPENCRAFT_ENTRY" plugins update marketplace-shortcut --dry-run
node "$OPENCRAFT_ENTRY" plugins update marketplace-shortcut
node "$OPENCRAFT_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
