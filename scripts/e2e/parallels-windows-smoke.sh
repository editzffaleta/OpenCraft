...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-opencraft-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://opencraft.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/opencraft-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-opencraft-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://opencraft.ai/install.ps1
...
guest_run_opencraft() {
  ...
  stdout_name="opencraft-stdout-$RANDOM-$RANDOM.log"
  stderr_name="opencraft-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\opencraft.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "opencraft-parallels-smoke",
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/opencraft-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\opencraft.cmd') --version
EOF
)"
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_opencraft "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="opencraft-onboard-$RANDOM-$RANDOM.ps1"
  log_name="opencraft-onboard-$RANDOM-$RANDOM.log"
  done_name="opencraft-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$opencraft = Join-Path \$env:APPDATA 'npm\opencraft.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$opencraft, \$log)
...
verify_gateway() {
  guest_run_opencraft "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_opencraft "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_opencraft "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_opencraft "" "" gateway status --deep
}

verify_turn() {
  guest_run_opencraft "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opencraft-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opencraft-main-upgrade.tgz" || return $?
