...
function Write-Banner {
    ...
    Write-Host "${MUTED}  All your chats, one OpenCraft.$NC" -Level info
    Write-Host ""
}
...
function Install-OpenCraftNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing OpenCraft ($installSpec)..." -Level info
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            ...
            Install-OpenCraftGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenCraft via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-OpenCraftNpm -Target $Tag)) {
                exit 1
            }
        }
    }
