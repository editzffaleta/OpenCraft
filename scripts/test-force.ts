...
function runTests() {
  const isolatedLock =
    process.env.OPENCRAFT_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `opencraft-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      OPENCRAFT_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.OPENCRAFT_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
