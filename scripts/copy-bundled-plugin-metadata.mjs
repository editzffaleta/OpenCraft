...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "opencraft.plugin.json");
    const distPluginDir = path.join(distExtensionsRoot, dirent.name);
    const distManifestPath = path.join(distPluginDir, "opencraft.plugin.json");
    ...
    if (packageJson.opencraft && "extensions" in packageJson.opencraft) {
      packageJson.opencraft = {
        ...packageJson.opencraft,
        extensions: rewritePackageExtensions(packageJson.opencraft.extensions),
        ...(typeof packageJson.opencraft.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.opencraft.setupEntry) }
          : {}),
      };
    }
