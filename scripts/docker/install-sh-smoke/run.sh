...
INSTALL_URL="${OPENCRAFT_INSTALL_URL:-https://opencraft.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENCRAFT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENCRAFT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="opencraft"
PACKAGE_NAME="${OPENCRAFT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENCRAFT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENCRAFT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENCRAFT_INSTALL_LATEST_OUT:-}"
fi
