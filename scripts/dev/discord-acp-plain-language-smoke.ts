...
type DriverMode = "token" | "webhook" | "opencraft";
...
function resolveStateDir(): string {
  const override = process.env.OPENCRAFT_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPENCRAFT_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".opencraft");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver opencraft] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OpenCraft spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|opencraft> Driver transport mode (default: token)\n" +
    ...
    "  --opencraft-bin <path>        OpenCraft CLI binary for driver=opencraft (default: opencraft)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPENCRAFT_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPENCRAFT_DISCORD_SMOKE_DRIVER\n" +
    "  OPENCRAFT_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPENCRAFT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPENCRAFT_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPENCRAFT_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPENCRAFT_DISCORD_SMOKE_AGENT\n" +
    "  OPENCRAFT_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPENCRAFT_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPENCRAFT_DISCORD_SMOKE_POLL_MS\n" +
    "  OPENCRAFT_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPENCRAFT_DISCORD_SMOKE_OPENCRAFT_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "opencraft"
        ? "opencraft"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPENCRAFT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OPENCRAFT_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPENCRAFT_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPENCRAFT_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPENCRAFT_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const opencraftBin =
    resolveArg("--opencraft-bin") || process.env.OPENCRAFT_DISCORD_SMOKE_OPENCRAFT_BIN || "opencraft";
...
async function opencraftCliJson<T>(params: { opencraftBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.opencraftBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`opencraft ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await opencraftCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    opencraftBin: params.opencraftBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "opencraft") {
    return await readMessagesWithOpenclaw({
      opencraftBin: params.args.opencraftBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `opencraft-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await opencraftCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        opencraftBin: args.opencraftBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("opencraft message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
