...
docker run --rm -t \
  ...
  -e OPENCRAFT_LIVE_GATEWAY_MODELS="${OPENCRAFT_LIVE_GATEWAY_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MODELS:-modern}}" \
  -e OPENCRAFT_LIVE_GATEWAY_PROVIDERS="${OPENCRAFT_LIVE_GATEWAY_PROVIDERS:-${CLAWDBOT_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e OPENCRAFT_LIVE_GATEWAY_MAX_MODELS="${OPENCRAFT_LIVE_GATEWAY_MAX_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MAX_MODELS:-24}}" \
  -e OPENCRAFT_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${OPENCRAFT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \
  -v "$ROOT_DIR":/src:ro \
