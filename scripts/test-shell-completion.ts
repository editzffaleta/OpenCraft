 ...
 * `opencraft update`. Use it to verify the completion installation flow
...
\`opencraft update\`, \`opencraft doctor\`, and \`opencraft onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenCraft Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opencraft completion --install\` later to enable.`));
    return;
  }
