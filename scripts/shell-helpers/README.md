...
Inspired by Simon Willison's [Running OpenCraft in Docker](https://til.simonwillison.net/llms/opencraft-docker).
...
On first command, ClawDock auto-detects your OpenCraft directory:

- Checks common paths (`~/opencraft`, `~/workspace/opencraft`, etc.)
...
```bash
opencraft channels login --channel whatsapp --verbose
```
...
2. Configure it in the OpenCraft config
...
- Bash or Zsh shell
- OpenCraft project (from `docker-setup.sh`)
