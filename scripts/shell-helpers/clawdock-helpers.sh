...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OpenCraft at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/opencraft"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OpenCraft not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d opencraft-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart opencraft-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f opencraft-gateway
}
...
clawdock-cli() {
  _clawdock_compose run --rm opencraft-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build opencraft-gateway
}
...
clawdock-fix-token() {
  ...
  _clawdock_compose restart opencraft-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm opencraft-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec opencraft-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'opencraft config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (opencraft alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to opencraft project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.opencraft)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opencraft channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opencraft status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.opencraft.ai${_CLR_RESET}"
  echo ""
}
