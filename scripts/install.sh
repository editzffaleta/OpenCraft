...
install_opencraft() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ...
        ui_info "Installing OpenCraft (${OPENCLAW_VERSION})"
    fi
    local install_spec=""
    ...
    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "opencraft" ]]; then
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.editzffaleta/OpenCraft.json}"
