...
# OpenCraft Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash
...
GUM_VERSION="${OPENCRAFT_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 OpenCraft Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://opencraft.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$OPENCRAFT_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.opencraft.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_opencraft_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.opencraft-* "$npm_root"/opencraft 2>/dev/null || true
}

extract_opencraft_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_opencraft_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/opencraft" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/opencraft"|"/usr/local/bin/opencraft")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/opencraft/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale opencraft symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing opencraft binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing OpenCraft package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_opencraft_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "OpenCraft npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*opencraft" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_opencraft_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_opencraft_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "OpenCraft npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an opencraft binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "OPENCRAFT_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "OPENCRAFT_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "OPENCRAFT_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "OPENCRAFT_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "OPENCRAFT_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "OPENCRAFT_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "OPENCRAFT_BETA" "CLAWDBOT_BETA"
map_legacy_env "OPENCRAFT_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "OPENCRAFT_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "OPENCRAFT_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "OPENCRAFT_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "OPENCRAFT_PROFILE" "CLAWDBOT_PROFILE"
...
pick_tagline() {
    ...
    if [[ -n "${OPENCRAFT_TAGLINE_INDEX:-}" ]]; then
        if [[ "${OPENCRAFT_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((OPENCRAFT_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${OPENCRAFT_NO_ONBOARD:-0}
NO_PROMPT=${OPENCRAFT_NO_PROMPT:-0}
DRY_RUN=${OPENCRAFT_DRY_RUN:-0}
INSTALL_METHOD=${OPENCRAFT_INSTALL_METHOD:-}
OPENCRAFT_VERSION=${OPENCRAFT_VERSION:-latest}
USE_BETA=${OPENCRAFT_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/opencraft"
GIT_DIR=${OPENCRAFT_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${OPENCRAFT_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${OPENCRAFT_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${OPENCRAFT_VERBOSE:-0}"
VERIFY_INSTALL="${OPENCRAFT_VERIFY_INSTALL:-0}"
OPENCRAFT_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
OpenCraft installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/opencraft)
...
Environment variables:
  OPENCRAFT_INSTALL_METHOD=git|npm
  OPENCRAFT_VERSION=latest|next|main|<semver>|<spec>
  OPENCRAFT_BETA=0|1
  OPENCRAFT_GIT_DIR=...
  OPENCRAFT_GIT_UPDATE=0|1
  OPENCRAFT_NO_PROMPT=1
  OPENCRAFT_VERIFY_INSTALL=1
  OPENCRAFT_DRY_RUN=1
  OPENCRAFT_NO_ONBOARD=1
  OPENCRAFT_VERBOSE=1
  OPENCRAFT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected OpenCraft checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a OpenCraft source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_opencraft_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"opencraft"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://opencraft.ai/install.sh | bash"
}
...
ensure_opencraft_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/opencraft" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/opencraft" ]]; then
        ln -sf "$npm_root/opencraft/dist/entry.js" "${npm_bin}/opencraft"
        ui_info "Created opencraft bin link at ${npm_bin}/opencraft"
    fi
    return 0
}

# Check for existing OpenCraft installation
check_existing_opencraft() {
    if [[ -n "$(type -P opencraft 2>/dev/null || true)" ]]; then
        ui_info "Existing OpenCraft installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make opencraft show as \"command not found\" in new terminals."
...
warn_opencraft_not_found() {
    ui_warn "Installed, but opencraft is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t opencraft 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named opencraft; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_opencraft_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P opencraft 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/opencraft" ]]; then
        echo "${npm_bin}/opencraft"
        return 0
    fi
...
install_opencraft_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing OpenCraft from GitHub (${repo_url})"
    fi
    ...
    cat > "$HOME/.local/bin/opencraft" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/opencraft"
    ui_success "OpenCraft wrapper installed to \$HOME/.local/bin/opencraft"
...
install_opencraft() {
    local package_name="opencraft"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            OPENCRAFT_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="opencraft"
        else
            OPENCRAFT_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${OPENCRAFT_VERSION}" ]]; then
        OPENCRAFT_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${OPENCRAFT_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${OPENCRAFT_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing OpenCraft v${resolved_version}"
    else
        ui_info "Installing OpenCraft (${OPENCRAFT_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${OPENCRAFT_VERSION}")"

    if ! install_opencraft_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_opencraft_paths
        install_opencraft_npm "${install_spec}"
    fi

    if [[ "${OPENCRAFT_VERSION}" == "latest" && "${package_name}" == "opencraft" ]]; then
        if ! resolve_opencraft_bin &> /dev/null; then
            ui_warn "npm install opencraft@latest failed; retrying opencraft@next"
            cleanup_npm_opencraft_paths
            install_opencraft_npm "opencraft@next"
        fi
    fi

    ensure_opencraft_bin_link || true

    ui_success "OpenCraft installed"
}
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (opencraft not on PATH yet)"
        warn_opencraft_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${OPENCRAFT_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.opencraft/workspace-${profile}"
    else
        echo "${HOME}/.opencraft/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run opencraft onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but opencraft not on PATH; skipping onboarding"
        warn_opencraft_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run opencraft onboard to retry"
        return
    }
}
...
if ! declare -F extract_opencraft_semver >/dev/null 2>&1; then
...
resolve_opencraft_version() {
    ...
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_opencraft_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/opencraft/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/opencraft/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: opencraft not on PATH yet"
        warn_opencraft_not_found
        return 1
    fi

    run_quiet_step "Checking OpenCraft version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: opencraft gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_opencraft_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found OpenCraft checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set OPENCRAFT_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_opencraft; then
        is_upgrade=true
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if npm list -g opencraft &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g opencraft 2>/dev/null || true
        ...
        install_opencraft_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/opencraft" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/opencraft"
    ...
    installed_version=$(resolve_opencraft_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 OpenCraft installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 OpenCraft installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/opencraft"
        ui_kv "Update command" "opencraft update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://opencraft.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running opencraft doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                OPENCRAFT_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                OPENCRAFT_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                OPENCRAFT_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run opencraft doctor and opencraft plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (opencraft not on PATH yet)"
                    warn_opencraft_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run opencraft onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v opencraft &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (opencraft daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if OPENCRAFT_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: opencraft daemon restart"
                fi
            fi
        fi
    fi
