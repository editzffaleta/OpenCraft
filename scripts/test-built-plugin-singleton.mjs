...
const { clearPluginCommands, getPluginCommandSpecs, loadOpenCraftPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadOpenCraftPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opencraft-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@opencraft/build-smoke-plugin",
      type: "module",
      opencraft: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'opencraft/plugin-sdk';",
...
const registry = loadOpenCraftPlugins({
  ...
  env: {
    ...process.env,
    OPENCRAFT_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
    OPENCRAFT_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
  },
  config: {
    plugins: {
      enabled: true,
