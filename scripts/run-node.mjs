...
const extensionRestartMetadataFiles = new Set(["opencraft.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.OPENCRAFT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENCRAFT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[opencraft] ${message}\n`);
};

const runOpenCraft = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["opencraft.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
