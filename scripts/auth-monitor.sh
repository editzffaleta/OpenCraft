...
#
# Suggested cron: */30 * * * * /home/admin/opencraft/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenCraft notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opencraft-alerts)
...
STATE_FILE="$HOME/.opencraft/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenCraft if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opencraft
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenCraft to $NOTIFY_PHONE..."
            opencraft send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenCraft is down. Run: ssh l36 '~/opencraft/scripts/mobile-reauth.sh'" "urgent"
    exit 1
