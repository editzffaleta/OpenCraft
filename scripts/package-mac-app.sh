...
# Build and bundle OpenCraft into a minimal .app we can open.
# Outputs to dist/OpenCraft.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenCraft.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opencraft/opencraft/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenCraft/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenCraftBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenCraftGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenCraft"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenCraft"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenCraft"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenCraft" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenCraft/Resources/OpenCraft.icns" "$APP_ROOT/Contents/Resources/OpenCraft.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenCraft/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenCraftKit resources"
OPENCRAFTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenCraftKit_OpenCraftKit.bundle"
if [ -d "$OPENCRAFTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenCraftKit_OpenCraftKit.bundle"
  cp -R "$OPENCRAFTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenCraftKit_OpenCraftKit.bundle"
else
  echo "WARN: OpenCraftKit resource bundle not found at $OPENCRAFTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenCraft"
killall -q OpenCraft 2>/dev/null || true
