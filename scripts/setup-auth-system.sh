#!/bin/bash
# Setup OpenCraft Auth Management System
...
echo "=== OpenCraft Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opencraft-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenCraft message: Send warning via OpenCraft itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opencraft-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opencraft-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opencraft-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opencraft-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status opencraft-auth-monitor.timer"
echo ""
echo "Quick commands:"
