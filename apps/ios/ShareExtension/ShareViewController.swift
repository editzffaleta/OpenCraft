...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.opencraft.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenCraft", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenCraft gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenCraft.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenCraftShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenCraft is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenCraft Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("opencraft-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
