...
final class RemindersService: RemindersServicing {
    func list(params: OpenCraftRemindersListParams) async throws -> OpenCraftRemindersListPayload {
        ...
        let payload: [OpenCraftReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenCraftReminderPayload(
        ...
        return OpenCraftRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenCraftRemindersAddParams) async throws -> OpenCraftRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenCraftReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenCraftRemindersAddPayload(reminder: payload)
    }
