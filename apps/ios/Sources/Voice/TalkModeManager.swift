...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENCRAFT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenCraftTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.opencraft", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenCraftTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenCraftTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenCraftTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenCraftTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenCraftTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenCraftTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenCraftTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenCraftTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenCraftTalkPTTStopPayload) {
