...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OpenCraftCapability.canvas.rawValue, OpenCraftCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OpenCraftCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OpenCraftCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OpenCraftLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OpenCraftCapability.location.rawValue) }

        caps.append(OpenCraftCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(OpenCraftCapability.watch.rawValue)
        }
        caps.append(OpenCraftCapability.photos.rawValue)
        caps.append(OpenCraftCapability.contacts.rawValue)
        caps.append(OpenCraftCapability.calendar.rawValue)
        caps.append(OpenCraftCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(OpenCraftCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OpenCraftScreenCommand.record.rawValue,
            OpenCraftSystemCommand.notify.rawValue,
            OpenCraftChatCommand.push.rawValue,
        ...
        if caps.contains(OpenCraftCapability.camera.rawValue) {
            commands.append(OpenCraftCameraCommand.list.rawValue)
            commands.append(OpenCraftCameraCommand.snap.rawValue)
            commands.append(OpenCraftCameraCommand.clip.rawValue)
        }
        if caps.contains(OpenCraftCapability.location.rawValue) {
            commands.append(OpenCraftLocationCommand.get.rawValue)
        }
        if caps.contains(OpenCraftCapability.device.rawValue) {
            commands.append(OpenCraftDeviceCommand.status.rawValue)
            commands.append(OpenCraftDeviceCommand.info.rawValue)
        }
        if caps.contains(OpenCraftCapability.watch.rawValue) {
            commands.append(OpenCraftWatchCommand.status.rawValue)
            commands.append(OpenCraftWatchCommand.notify.rawValue)
        }
        if caps.contains(OpenCraftCapability.photos.rawValue) {
            commands.append(OpenCraftPhotosCommand.latest.rawValue)
        }
        if caps.contains(OpenCraftCapability.contacts.rawValue) {
            commands.append(OpenCraftContactsCommand.search.rawValue)
            commands.append(OpenCraftContactsCommand.add.rawValue)
        }
        if caps.contains(OpenCraftCapability.calendar.rawValue) {
            commands.append(OpenCraftCalendarCommand.events.rawValue)
            commands.append(OpenCraftCalendarCommand.add.rawValue)
        }
        if caps.contains(OpenCraftCapability.reminders.rawValue) {
            commands.append(OpenCraftRemindersCommand.list.rawValue)
            commands.append(OpenCraftRemindersCommand.add.rawValue)
        }
        if caps.contains(OpenCraftCapability.motion.rawValue) {
            commands.append(OpenCraftMotionCommand.activity.rawValue)
            commands.append(OpenCraftMotionCommand.pedometer.rawValue)
        }
