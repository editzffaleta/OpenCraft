...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = OpenCraftLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = OpenCraftLocationMode.off.rawValue
    ...
    private let gatewayLogger = Logger(subsystem: "ai.opencraft.ios", category: "GatewaySettings")
    ...
    var body: some View {
        NavigationStack {
                ...
                Section {
                    DisclosureGroup(isExpanded: self.$gatewayExpanded) {
                        if !self.isGatewayConnected {
                            Text(
                                "1. Open a chat with your OpenCraft agent and send /pair\n"
                ...
                Section("Device") {
                    DisclosureGroup("Features") {
                        ...
                        self.featureToggle(
                            ...
                            help: "Enables voice conversation mode with your connected OpenCraft agent.") { newValue in
                        ...
                        self.featureToggle(
                            ...
                            help: "Allows the gateway to request photos or short video clips "
                                + "while OpenCraft is foregrounded."
                        )
                        ...
                        HStack(spacing: 8) {
                            ...
                            Button {
                                self.activeFeatureHelp = FeatureHelp(
                                    ...
                                    message: "Controls location permissions for OpenCraft. "
                        ...
                        Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                            Text("Off").tag(OpenCraftLocationMode.off.rawValue)
                            Text("While Using").tag(OpenCraftLocationMode.whileUsing.rawValue)
                            Text("Always").tag(OpenCraftLocationMode.always.rawValue)
                        }
                        .labelsHidden()
                        ...
                        self.featureToggle(
                            ...
                            help: "Keeps the screen awake while OpenCraft is open.")
                    ...
                    DisclosureGroup("Device Info") {
                        ...
                        LabeledContent("OpenCraft", value: DeviceInfoHelper.openCraftVersionString())
                    }
                }
            }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = OpenCraftLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Go back to your OpenCraft chat and run /pair approve, then tap Connect again."
        }
