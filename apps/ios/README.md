...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `opencraft.json`. `OPENCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
