...
@MainActor
public struct OpenCraftChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: OpenCraftChatViewModel
    ...
    public init(
        viewModel: OpenCraftChatViewModel,
    ...
    private var visibleMessages: [OpenCraftChatMessage] {
        let base: [OpenCraftChatMessage]
    ...
    private func mergeToolResults(in messages: [OpenCraftChatMessage]) -> [OpenCraftChatMessage] {
        ...
        for message in messages {
            ...
            let merged = OpenCraftChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: OpenCraftChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: OpenCraftChatMessage) -> Bool {
    ...
    private func primaryText(in message: OpenCraftChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: OpenCraftChatMessage) -> Bool {
    ...
    private func toolCalls(in message: OpenCraftChatMessage) -> [OpenCraftChatMessageContent] {
    ...
    private func inlineToolResults(in message: OpenCraftChatMessage) -> [OpenCraftChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: OpenCraftChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OpenCraftChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(OpenCraftChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
