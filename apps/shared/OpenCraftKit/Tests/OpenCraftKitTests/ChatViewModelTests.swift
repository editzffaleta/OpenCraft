...
import Foundation
import Testing
@testable import OpenCraftChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OpenCraftChatHistoryPayload
{
    OpenCraftChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OpenCraftChatSessionEntry {
    OpenCraftChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> OpenCraftChatSessionEntry
{
    OpenCraftChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> OpenCraftChatModelChoice {
    OpenCraftChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OpenCraftChatHistoryPayload],
    sessionsResponses: [OpenCraftChatSessionsListResponse] = [],
    modelResponses: [[OpenCraftChatModelChoice]] = [],
    ...
    -> (TestChatTransport, OpenCraftChatViewModel)
{
...
private func sendUserMessage(_ vm: OpenCraftChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OpenCraftChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenCraftChatHistoryPayload]
    private let sessionsResponses: [OpenCraftChatSessionsListResponse]
    private let modelResponses: [[OpenCraftChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<OpenCraftChatTransportEvent>
    private let continuation: AsyncStream<OpenCraftChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<OpenCraftChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenCraftChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenCraftChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [OpenCraftChatAttachmentPayload]) async throws -> OpenCraftChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return OpenCraftChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenCraftChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenCraftChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [OpenCraftChatModelChoice] {
    ...
    func emit(_ evt: OpenCraftChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = OpenCraftChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = OpenCraftChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OpenCraftChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(OpenCraftChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == OpenCraftChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = OpenCraftChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OpenCraftChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = OpenCraftChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = OpenCraftChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = OpenCraftChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { OpenCraftChatViewModel(sessionKey: "main", transport: transport) }
