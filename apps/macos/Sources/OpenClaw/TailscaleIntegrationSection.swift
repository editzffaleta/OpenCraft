...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.editzffaleta/OpenCraft.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.editzffaleta/OpenCraft.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.editzffaleta/OpenCraft.json. Restart the gateway to apply."
        }
