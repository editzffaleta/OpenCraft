...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.opencraft.mac`) even if the shipped
    ...
    {
        if environment["OPENCRAFT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "opencraft.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "opencraft.nixMode") { return true }
