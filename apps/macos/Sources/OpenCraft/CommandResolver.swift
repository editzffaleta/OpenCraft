...
enum CommandResolver {
    private static let projectRootDefaultsKey = "opencraft.gatewayProjectRootPath"
    private static let helperName = "opencraft"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let opencraftEntry = root.appendingPathComponent("opencraft.mjs").path
        if FileManager().isReadableFile(atPath: opencraftEntry) { return opencraftEntry }
        let binEntry = root.appendingPathComponent("bin/opencraft.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/opencraft")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let opencraftPaths = self.opencraftManagedPaths(home: home)
        if !opencraftPaths.isEmpty {
            extras.insert(contentsOf: opencraftPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + opencraftPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func opencraftManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".opencraft"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func opencraftExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenCraftExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("opencraft.mjs").path,
            root.appendingPathComponent("bin/opencraft.js").path,
        ]
    ...
    static func hasAnyOpenCraftInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.opencraftExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func opencraftNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let opencraftPath = self.projectOpenCraftExecutable(projectRoot: root) {
            return [opencraftPath, subcommand] + extraArgs
        }
        if let opencraftPath = self.opencraftExecutable(searchPaths: searchPaths) {
            return [opencraftPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "opencraft", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            opencraft entrypoint missing (looked for dist/index.js or opencraft.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real opencraft CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/opencraft"
        ...
        \(cliSection)
        \(projectSection)
        if command -v opencraft >/dev/null 2>&1; then
          CLI="$(command -v opencraft)"
          opencraft \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/opencraft.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/opencraft.mjs"
            node "$PRJ/opencraft.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/opencraft.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/opencraft.js"
            node "$PRJ/bin/opencraft.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent opencraft"
          pnpm --silent opencraft \(quotedArgs);
        else
          echo "opencraft CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenCraftConfigFile.loadDict()
