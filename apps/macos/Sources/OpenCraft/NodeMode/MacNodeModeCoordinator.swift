...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.opencraft", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OpenCraftNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [OpenCraftCapability.canvas.rawValue, OpenCraftCapability.screen.rawValue]
        if OpenCraftConfigFile.browserControlEnabled() {
            caps.append(OpenCraftCapability.browser.rawValue)
        }
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(OpenCraftCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if OpenCraftLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(OpenCraftCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            OpenCraftSystemCommand.which.rawValue,
            OpenCraftSystemCommand.run.rawValue,
            OpenCraftSystemCommand.execApprovalsGet.rawValue,
            OpenCraftSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(OpenCraftCapability.browser.rawValue) {
            commands.append(OpenCraftBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(OpenCraftCapability.camera.rawValue) {
        ...
        if capsSet.contains(OpenCraftCapability.location.rawValue) {
