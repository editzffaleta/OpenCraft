...
struct MacGatewayChatTransport: OpenCraftChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> OpenCraftChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(OpenCraftChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            OpenCraftChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return OpenCraftChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenCraftChatTransportEvent? {
        switch push {
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    OpenCraftChatEventPayload.self,
    ...
    private static func mapModelChoice(_ model: OpenCraftProtocol.ModelChoice) -> OpenCraftChatModelChoice {
        OpenCraftChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenCraftChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenCraftChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenCraftChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: OpenCraftChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenCraftChatView>) -> NSViewController
    {
