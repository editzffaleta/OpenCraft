...
final class PeekabooBridgeHostCoordinator {
    ...
    private static var opencraftSocketPath: String {
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenCraftPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.opencraftSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.opencraftSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.opencraftSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.opencraftSocketPath)
        } catch {
...
@MainActor
private final class OpenCraftPeekabooBridgeServices: PeekabooBridgeServiceProviding {
