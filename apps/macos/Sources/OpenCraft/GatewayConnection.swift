...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: OpenCraftProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> OpenCraftSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return OpenCraftSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> OpenCraftChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [OpenCraftChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> OpenCraftChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
