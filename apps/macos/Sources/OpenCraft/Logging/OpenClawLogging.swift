...
enum OpenCraftLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenCraftOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenCraftFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenCraftLogging.bootstrapIfNeeded()
        let label = OpenCraftLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenCraftOSLogHandler: AppLogLevelBackedHandler {
...
struct OpenCraftFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenCraftLogging.parseLabel(self.label)
