...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenCraftProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.opencraft", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenCraftKit.AnyCodable]()) {
                $0[$1.key] = OpenCraftKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenCraftProtocol.AnyCodable?) -> [String: OpenCraftProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenCraftProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenCraftKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenCraftProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("opencraft.control.heartbeat")
    static let controlAgentEvent = Notification.Name("opencraft.control.agent")
}
