...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.opencraft", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenCraftSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenCraftSessionPreviewEntry,
    ...
    private static func cache(payload: OpenCraftSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenCraftChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenCraftKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenCraftKit.AnyCodable]) -> [OpenCraftChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenCraftChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenCraftChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenCraftChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenCraftChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenCraftChatMessage) -> String? {
