...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENCRAFT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/opencraft")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("opencraft-stdout.log")
    }
    ...
    /// Returns the newest log file under /tmp/opencraft/ (rolling or stdout), or nil if none exist.
