...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.opencraft/opencraft.json. Prefer OPENCRAFT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.opencraft/opencraft.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.opencraft/opencraft.json. Restart the gateway to apply."
        }
