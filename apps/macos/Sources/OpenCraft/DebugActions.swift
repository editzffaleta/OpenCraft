...
enum DebugActions {
    private static let verboseDefaultsKey = "opencraft.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "opencraft.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenCraftPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenCraft", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenCraftPaths.configURL
        guard
