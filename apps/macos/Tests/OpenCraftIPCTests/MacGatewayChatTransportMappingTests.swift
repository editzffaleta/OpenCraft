...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenCraftProtocol.AnyCodable(["ok": OpenCraftProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenCraftProtocol.AnyCodable(["ok": OpenCraftProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = OpenCraftProtocol.AnyCodable([
            "runId": OpenCraftProtocol.AnyCodable("run-1"),
            "sessionKey": OpenCraftProtocol.AnyCodable("main"),
            "state": OpenCraftProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenCraftProtocol.AnyCodable(["a": OpenCraftProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
