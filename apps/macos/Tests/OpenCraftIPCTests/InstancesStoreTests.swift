...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: OpenCraftProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenCraftProtocol.AnyCodable] = [
            "presence": .init([OpenCraftProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenCraftProtocol.AnyCodable(payloadMap)
