...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["opencraft"]
        let text = "hey opencraft"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey opencraft do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["opencraft"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "opencraft", start: 0.2, duration: 0.1, range: transcript.range(of: "opencraft")),
