import type { SlackSlashCommandConfig } from "opencraft/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "opencraft");
  const name = normalizedName || "opencraft";
  return {
