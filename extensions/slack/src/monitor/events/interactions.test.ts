...
describe("registerSlackInteractionEvents", () => {
  ...
  it("registers a matcher that accepts plugin action ids beyond the OpenCraft prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("opencraft:verify")).toBe(true);
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "opencraft:reply_button" }],
            },
          ],
        },
      },
