...
// Prefix for OpenCraft-generated action IDs to scope our handler
const OPENCRAFT_ACTION_PREFIX = "opencraft:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${OPENCRAFT_ACTION_PREFIX}`);

  // Handle OpenCraft modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
