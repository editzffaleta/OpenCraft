...
function setZalouserDmPolicy(cfg: OpenCraftConfig, dmPolicy: DmPolicy): OpenCraftConfig {
...
function ensureZalouserPluginEnabled(cfg: OpenCraftConfig): OpenCraftConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("opencraft directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
...
const zalouserDmPolicy: ChannelSetupDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZalouserDmPolicy(cfg as OpenCraftConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? (normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID)
        : resolveDefaultZalouserAccountId(cfg as OpenCraftConfig);
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as OpenCraftConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("opencraft directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as OpenCraftConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as OpenCraftConfig, accountId, resolved as string[]),
  },
