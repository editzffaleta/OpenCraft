...
import type { OpenCraftConfig, PluginRuntime } from "opencraft/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): OpenCraftConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
