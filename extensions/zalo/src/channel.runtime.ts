...
import { PAIRING_APPROVED_MESSAGE } from "opencraft/plugin-sdk/zalo";
...
export async function notifyZaloPairingApproval(params: {
  cfg: import("opencraft/plugin-sdk/zalo").OpenCraftConfig;
  id: string;
}) {
...
export async function startZaloGatewayAccount(
  ctx: Parameters<
    NonNullable<
      NonNullable<import("opencraft/plugin-sdk/zalo").ChannelPlugin["gateway"]>["startAccount"]
    >
  >[0],
) {
