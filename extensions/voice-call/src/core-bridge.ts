...
function resolveOpenCraftRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENCRAFT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENCRAFT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenCraftRoot(), "dist", "extensionAPI.js");
