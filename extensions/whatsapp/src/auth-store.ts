...
import { info, success } from "opencraft/plugin-sdk/runtime-env";
...
import type { WebChannel } from "opencraft/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "opencraft/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("opencraft channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
