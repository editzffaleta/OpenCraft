...
const CONFIG_KEY = Symbol.for("opencraft:testConfigMock");
...
vi.mock("opencraft/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opencraft/plugin-sdk/media-runtime")>();
...
vi.mock("@whiskeysockets/baileys", () => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("opencraft:lastSocket")] =
    created.lastSocket;
  return created.mod;
});
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("opencraft:lastSocket")];
  if (typeof getter === "function") {
