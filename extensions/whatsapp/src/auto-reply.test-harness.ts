...
import * as ssrf from "opencraft/plugin-sdk/infra-runtime";
import { resetInboundDedupe } from "opencraft/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "opencraft/plugin-sdk/runtime-env";
...
vi.mock("opencraft/plugin-sdk/agent-runtime", () => ({
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-session-"));
