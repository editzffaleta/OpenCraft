...
import { getChildLogger } from "opencraft/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "opencraft/plugin-sdk/text-runtime";
import { toWhatsappJid } from "opencraft/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: OpenCraftConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
