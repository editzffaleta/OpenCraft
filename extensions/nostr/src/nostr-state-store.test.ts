...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENCRAFT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-nostr-"));
  process.env.OPENCRAFT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(resolveHome(), ".opencraft");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENCRAFT_STATE_DIR = previous;
    }
