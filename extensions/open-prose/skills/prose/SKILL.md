...
metadata: { "opencraft": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenCraft `sessions_spawn`
- **File I/O** == OpenCraft `read`/`write`
- **Remote fetch** == OpenCraft `web_fetch` (or `exec` with curl when POST is required)
