...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__opencraft_channel_id";
...
type InteractionCallbackConfig = Pick<OpenCraftConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "opencraft-mattermost-interactions").update(botToken).digest("hex");
}
