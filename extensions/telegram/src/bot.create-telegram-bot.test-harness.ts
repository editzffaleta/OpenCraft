...
import { resetInboundDedupe } from "opencraft/plugin-sdk/reply-runtime";
import type { MsgContext } from "opencraft/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "opencraft/plugin-sdk/reply-runtime";
import type { MockFn } from "opencraft/plugin-sdk/testing";
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/opencraft-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("opencraft/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
vi.mock("opencraft/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opencraft/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
const skillCommandsHoisted = vi.hoisted(() => ({
  ...
  replySpy: vi.fn(async (_ctx: MsgContext, opts?: GetReplyOptions) => {
    ...
    (
      ...
      configOverride?: OpenCraftConfig,
...
vi.mock("opencraft/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opencraft/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.mock("opencraft/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opencraft/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: OpenCraftConfig = {
  agents: {
    defaults: {
