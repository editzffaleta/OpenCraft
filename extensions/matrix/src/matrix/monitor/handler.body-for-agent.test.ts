...
import type { PluginRuntime, RuntimeEnv, RuntimeLogger } from "opencraft/plugin-sdk/matrix";
  ...
  it("stores sender-labeled BodyForAgent for group thread messages", async () => {
    ...
    const core = {
      channel: {
        ...
        session: {
          resolveStorePath: vi.fn().mockReturnValue("/tmp/opencraft-test-session.json"),
