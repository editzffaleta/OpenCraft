...
function getChannelConfig(cfg: OpenCraftConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: OpenCraftConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: OpenCraftConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: OpenCraftConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL OpenCraft uses to send replies back to Chat.",
      ],
