...
import { loadConfig, type OpenCraftConfig } from "opencraft/plugin-sdk/config-runtime";
...
import { maxBytesForKind } from "opencraft/plugin-sdk/media-runtime";
import { extensionForMime } from "opencraft/plugin-sdk/media-runtime";
...
import { loadWebMediaRaw } from "opencraft/plugin-sdk/web-media";
...
async function materializeVoiceMessageInput(mediaUrl: string): Promise<{ filePath: string }> {
  ...
  const tempDir = resolvePreferredOpenCraftTmpDir();
