  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/opencraft",
      },
    },
    session: { store: "/tmp/opencraft-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bopencraft\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "opencraft: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "OpenCraft" },
          },
        },
        eventPatch: {
