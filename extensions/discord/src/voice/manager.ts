...
import { resolveAgentDir } from "opencraft/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "opencraft/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "opencraft/plugin-sdk/agent-runtime";
...
import type { DiscordAccountConfig, TtsConfig } from "opencraft/plugin-sdk/config-runtime";
...
import { transcribeAudioFile } from "opencraft/plugin-sdk/media-understanding-runtime";
...
import { parseTtsDirectives } from "opencraft/plugin-sdk/speech";
import { textToSpeech } from "opencraft/plugin-sdk/speech-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: OpenCraftConfig; override?: TtsConfig }): {
  cfg: OpenCraftConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOpenCraftTmpDir(), "discord-voice-"));
