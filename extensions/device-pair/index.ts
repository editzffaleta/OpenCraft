...
import type { OpenCraftPluginApi } from "opencraft/plugin-sdk/device-pair";
import {
...
} from "opencraft/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: OpenCraftPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENCRAFT_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: OpenCraftPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: OpenCraftPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenCraft iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenCraft iOS app:",
            "",
            "```",
            qrAscii,
            "```",
