...
const DEFAULT_PACKAGE_NAME = "opencraft";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["opencraft", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "opencraft.mjs");
    ...
    const doctorStep = await runStep(
      step("opencraft doctor", doctorArgv, gitRoot, { OPENCRAFT_UPDATE_IN_PROGRESS: "1" }),
    );
