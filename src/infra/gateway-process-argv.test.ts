...
describe("isGatewayArgv", () => {
  ...
  it("matches the opencraft executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\opencraft.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/opencraft-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/opencraft-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\opencraft-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/opencraft/custom.js", "gateway"])).toBe(false);
