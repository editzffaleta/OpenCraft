...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__opencraft_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "opencraft-root");
...
describe("resolveOpenCraftPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    expect(resolveOpenCraftPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOpenCraftPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-opencraft moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-opencraft" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "opencraft" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveOpenCraftPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveOpenCraftPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveOpenCraftPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveOpenCraftPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-opencraft package roots", async () => {
    const pkgRoot = fx("not-opencraft");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-opencraft" }));

    expect(resolveOpenCraftPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOpenCraftPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOpenCraftPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
