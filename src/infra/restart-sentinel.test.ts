...
describe("restart sentinel", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["OPENCRAFT_STATE_DIR"]);
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-sentinel-"));
  ...
  it("formats summary, distinct reason, and doctor hint together", () => {
    const payload = {
      ...
      doctorHint: "Run opencraft doctor",
