...
describe("normalizePackageTagInput", () => {
  const packageNames = ["opencraft", "@opencraft/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("opencraft@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@opencraft/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("opencraft@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("opencraft", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @opencraft/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("opencrafter@beta", packageNames)).toBe("opencrafter@beta");
  });
});
