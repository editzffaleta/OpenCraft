 ...
 * returning control to the caller (typically `triggerOpenCraftRestart` →
...
/**
 * Parse opencraft gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("opencraft")) {
 ...
 * Returns only PIDs that belong to opencraft gateway processes (not the current process).
 */
