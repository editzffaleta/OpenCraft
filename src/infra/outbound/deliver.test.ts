...
import { resolvePreferredOpenCraftTmpDir } from "../tmp-opencraft-dir.js";
...
const telegramChunkConfig: OpenCraftConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: OpenCraftConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes OpenCraft tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredOpenCraftTmpDir()]),
      }),
    );
  });

  it("includes OpenCraft tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes OpenCraft tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes OpenCraft tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: OpenCraftConfig = { channels: { signal: { mediaMaxMb: 2 } } };
