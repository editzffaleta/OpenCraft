...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred OpenCraft tmp root", async () => {
      const tmpRoot = resolvePreferredOpenCraftTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideOpenCraft = path.join(os.tmpdir(), "outside-opencraft", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
