...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when OPENCRAFT_VERSION is not set", async () => {
  ...
  it("prefers OPENCRAFT_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over OPENCRAFT_SERVICE_VERSION when OPENCRAFT_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        OPENCRAFT_VERSION: " ",
        OPENCRAFT_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when OPENCRAFT_VERSION and OPENCRAFT_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        OPENCRAFT_VERSION: " ",
        OPENCRAFT_SERVICE_VERSION: "\t",
