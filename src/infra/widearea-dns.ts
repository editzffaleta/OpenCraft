...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENCRAFT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opencraft-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opencraft";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opencraft");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opencraft-gw");
  ...
  records.push(`_opencraft-gw._tcp IN PTR ${instanceLabel}._opencraft-gw._tcp`);
  records.push(`${instanceLabel}._opencraft-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opencraft-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opencraft-content-hash: ${contentHash}\n${contentBody}`;
}
