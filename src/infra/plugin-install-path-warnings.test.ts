...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/opencraft-matrix-missing",
        installPath: "/tmp/opencraft-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/opencraft-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "opencraft plugins install @opencraft/matrix",
        repoInstallCommand: "opencraft plugins install ./extensions/matrix",
      }),
    ).toEqual([
      "Matrix is installed from a custom path that no longer exists: /tmp/opencraft-matrix-missing",
      'Reinstall with "opencraft plugins install @opencraft/matrix".',
      'If you are running from a repo checkout, you can also use "opencraft plugins install ./extensions/matrix".',
    ]);
  });
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<opencraft plugins install @opencraft/matrix>" when you want to return to the standard Matrix plugin.',
      'If you are intentionally running from a repo checkout, reinstall that checkout explicitly with "<opencraft plugins install ./extensions/matrix>" after updates.',
    ]);
  });
});
