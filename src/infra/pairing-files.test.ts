...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/opencraft-state", "devices")).toEqual({
      dir: path.join("/tmp/opencraft-state", "devices"),
      pendingPath: path.join("/tmp/opencraft-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/opencraft-state", "devices", "paired.json"),
    });
  });
