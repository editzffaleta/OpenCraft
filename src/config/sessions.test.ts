...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENCRAFT_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-opencraft",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-opencraft#general");
  });
  ...
  it("derives session transcripts dir from OPENCRAFT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENCRAFT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPENCRAFT_STATE_DIR differs from stored paths", () => {
