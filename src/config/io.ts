...
import type { OpenCraftConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpenCraftVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  opencraft config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  opencraft config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OpenCraftConfig,
  pathSegments: string[],
): { changed: boolean; next: OpenCraftConfig } {
...
function coerceConfig(value: unknown): OpenCraftConfig {
...
function stampConfigVersion(cfg: OpenCraftConfig): OpenCraftConfig {
...
function warnIfConfigFromFuture(cfg: OpenCraftConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenCraftVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenCraft (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenCraftConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenCraftConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenCraftConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenCraftConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenCraftConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENCRAFT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENCRAFT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENCRAFT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENCRAFT_WATCH_SESSION === "string" &&
        deps.env.OPENCRAFT_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENCRAFT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENCRAFT_WATCH_COMMAND === "string" &&
        deps.env.OPENCRAFT_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENCRAFT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENCRAFT_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OpenCraftConfig | null = null;
let runtimeConfigSourceSnapshot: OpenCraftConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENCRAFT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENCRAFT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OpenCraftConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): OpenCraftConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: OpenCraftConfig;
  candidate: OpenCraftConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: OpenCraftConfig): OpenCraftConfig {
...
export function loadConfig(): OpenCraftConfig {
...
export async function readBestEffortConfig(): Promise<OpenCraftConfig> {
...
export async function writeConfigFile(
  ...
  // edits to opencraft.json remain visible (no stale snapshot).
}
