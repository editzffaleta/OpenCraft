...
function collectConfigEnvVarsByTarget(cfg?: OpenCraftConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OpenCraftConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: OpenCraftConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: OpenCraftConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.OPENCRAFT_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
