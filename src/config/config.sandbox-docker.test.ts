...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("opencraft-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "opencraft-sandbox-browser-global" },
      agentBrowser: { network: "opencraft-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("opencraft-sandbox-browser-agent");
  });
