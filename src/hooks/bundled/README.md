...
This directory contains hooks that ship with OpenCraft. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
2. Restart gateway: `pkill -9 -f 'opencraft.*gateway' && pnpm opencraft gateway`
3. Enable the hook: `opencraft hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opencraft.ai/automation/hooks
