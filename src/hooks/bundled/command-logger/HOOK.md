...
homepage: https://docs.opencraft.ai/automation/hooks#command-logger
metadata:
  {
    "opencraft":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opencraft/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opencraft/logs/commands.log ~/.opencraft/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opencraft`:
   ```
   /home/username/.opencraft/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
