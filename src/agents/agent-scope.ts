...
type AgentEntry = NonNullable<NonNullable<OpenCraftConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: OpenCraftConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OpenCraftConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OpenCraftConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: OpenCraftConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: OpenCraftConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: OpenCraftConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
