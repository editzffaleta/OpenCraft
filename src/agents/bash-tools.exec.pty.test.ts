...
test("exec sets OPENCRAFT_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-opencraft-shell", {
    command: "node -e \"process.stdout.write(process.env.OPENCRAFT_SHELL || '')\"",
    pty: true,
  });
