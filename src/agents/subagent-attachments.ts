...
function resolveAttachmentLimits(config: OpenCraftConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".opencraft", "attachments");
  const relDir = path.posix.join(".opencraft", "attachments", attachmentId);
