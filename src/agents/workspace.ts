...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENCRAFT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".opencraft", `workspace-${profile}`);
  }
  return path.join(home, ".opencraft", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".opencraft";
