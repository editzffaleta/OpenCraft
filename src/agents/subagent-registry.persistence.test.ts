...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPENCRAFT_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-subagent-"));
  ...
  it("uses isolated temp state when OPENCRAFT_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "opencraft-test-state"));
  });
});
