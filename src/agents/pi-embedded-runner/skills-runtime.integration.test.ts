...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("opencraft-bundled-");
  const workspaceDir = await createTempDir("opencraft-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: OpenCraftConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.OPENCRAFT_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
