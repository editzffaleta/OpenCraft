...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: OpenCraftConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/opencraft" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OpenCraftConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OpenCraftConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OpenCraftConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OpenCraftConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OpenCraftConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OpenCraftConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OpenCraftConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: OpenCraftConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/opencraft-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/opencraft");
  });

  it("uses OPENCRAFT_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "opencraft-home");
    vi.stubEnv("OPENCRAFT_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenCraftConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".opencraft", "workspace"));
  });

  it("uses OPENCRAFT_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "opencraft-home");
    vi.stubEnv("OPENCRAFT_HOME", home);
    // Clear state dir so it falls back to OPENCRAFT_HOME
    vi.stubEnv("OPENCRAFT_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OpenCraftConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".opencraft", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/opencraft-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/opencraft-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opencraft-agent-scope-"));
