...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    opencraft: process.env.OPENCRAFT_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.opencraft === undefined) {
      delete process.env.OPENCRAFT_GATEWAY_TOKEN;
    } else {
      process.env.OPENCRAFT_GATEWAY_TOKEN = envSnapshot.opencraft;
    }
  ...
  it("uses OPENCRAFT_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENCRAFT_GATEWAY_TOKEN = "local-env-token";
