...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.OPENCRAFT_TEST_FAST;
...
describe("opencraft-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    ...
    process.env.OPENCRAFT_TEST_FAST = fastModeEnv.previous;
  });
