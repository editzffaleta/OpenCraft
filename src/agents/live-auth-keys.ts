...
const GOOGLE_LIVE_SINGLE_KEY = "OPENCRAFT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENCRAFT_LIVE_ANTHROPIC_KEY",
    listVar: "OPENCRAFT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENCRAFT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENCRAFT_LIVE_${base}_KEY`;
