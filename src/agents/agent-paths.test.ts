...
describe("resolveOpenCraftAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          OPENCRAFT_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOpenCraftAgentDir();
  ...
  it("honors OPENCRAFT_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          OPENCRAFT_STATE_DIR: undefined,
          OPENCRAFT_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOpenCraftAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when OPENCRAFT_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers OPENCRAFT_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          OPENCRAFT_STATE_DIR: undefined,
          OPENCRAFT_AGENT_DIR: primaryOverride,
