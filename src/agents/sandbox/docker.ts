...
import { markOpenCraftExecEnv } from "../../infra/opencraft-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opencraft.sandbox=1");
  args.push("--label", `opencraft.sessionKey=${params.scopeKey}`);
  args.push("--label", `opencraft.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opencraft.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markOpenCraftExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "opencraft.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opencraft sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opencraft sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opencraft sandbox recreate --all");
}
