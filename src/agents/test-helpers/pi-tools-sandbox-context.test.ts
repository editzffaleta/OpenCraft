...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("opencraft-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("opencraft-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("opencraft-sbx-");
