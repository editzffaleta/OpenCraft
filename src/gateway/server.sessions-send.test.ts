...
const { createOpenCraftTools } = await import("../agents/opencraft-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpenCraftTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOpenCraftTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENCRAFT_GATEWAY_PORT", "OPENCRAFT_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENCRAFT_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENCRAFT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPENCRAFT_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createOpenCraftTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
