...
import { ensureOpenCraftModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, OpenCraftConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPENCRAFT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPENCRAFT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPENCRAFT_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.OPENCRAFT_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.OPENCRAFT_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.OPENCRAFT_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  cfg: OpenCraftConfig;
  agentDir: string;
}): OpenCraftConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<OpenCraftConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.OPENCRAFT_CONFIG_PATH,
    token: process.env.OPENCRAFT_GATEWAY_TOKEN,
    skipChannels: process.env.OPENCRAFT_SKIP_CHANNELS,
    skipGmail: process.env.OPENCRAFT_SKIP_GMAIL_WATCHER,
    skipCron: process.env.OPENCRAFT_SKIP_CRON,
    skipCanvas: process.env.OPENCRAFT_SKIP_CANVAS_HOST,
    agentDir: process.env.OPENCRAFT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OPENCRAFT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOpenCraftAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-live-state-"));
  process.env.OPENCRAFT_STATE_DIR = tempStateDir;
  ...
  process.env.OPENCRAFT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.opencraft-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOpenCraftAgentDir();
  const sanitizedCfg: OpenCraftConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-live-"));
  const tempConfigPath = path.join(tempDir, "opencraft.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.OPENCRAFT_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "OpenCraft live tool probe (local, safe): " +
    ...
    process.env.OPENCRAFT_CONFIG_PATH = previous.configPath;
    process.env.OPENCRAFT_GATEWAY_TOKEN = previous.token;
    process.env.OPENCRAFT_SKIP_CHANNELS = previous.skipChannels;
    process.env.OPENCRAFT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.OPENCRAFT_SKIP_CRON = previous.skipCron;
    process.env.OPENCRAFT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.OPENCRAFT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPENCRAFT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.OPENCRAFT_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via OPENCRAFT_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.opencraft-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
