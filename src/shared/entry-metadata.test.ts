...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://opencraft.ai " },
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.opencraft.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://opencraft.ai/install " },
      }),
    ).toEqual({
      homepage: "https://opencraft.ai/install",
    });
  });
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          ...
          url: "https://opencraft.ai/install",
        },
      }),
    ).toEqual({});
  });
