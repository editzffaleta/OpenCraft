...
let originalOpenCraftLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OPENCRAFT_TEST_FILE_LOG;
  originalOpenCraftLogLevel = process.env.OPENCRAFT_LOG_LEVEL;
  delete process.env.OPENCRAFT_TEST_FILE_LOG;
  delete process.env.OPENCRAFT_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OPENCRAFT_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpenCraftLogLevel === undefined) {
    delete process.env.OPENCRAFT_LOG_LEVEL;
  } else {
    process.env.OPENCRAFT_LOG_LEVEL = originalOpenCraftLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
