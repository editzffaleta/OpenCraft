...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop opencraft!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@opencraft_bot", { botUsername: "opencraft_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@opencraft_bot", { botUsername: "opencraft_bot" })).toBe(true);
