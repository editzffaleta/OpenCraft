...
type HomeEnvSnapshot = {
  ...
  OPENCRAFT_STATE_DIR: string | undefined;
  OPENCRAFT_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    OPENCRAFT_AGENT_DIR: process.env.OPENCRAFT_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.OPENCRAFT_STATE_DIR = path.join(home, ".opencraft");
    process.env.OPENCRAFT_AGENT_DIR = path.join(home, ".opencraft", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".opencraft", "agent");
