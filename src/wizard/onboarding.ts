...
export async function runOnboardingWizard(
  ...
  await prompter.intro("OpenCraft onboarding");
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Configuração inválida. Execute \`${formatCliCommand("opencraft doctor")}\` para corrigi-la, depois execute o onboarding novamente.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.OPENCRAFT_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN;
  try {
    ...
    await prompter.note(
      [
        "Não foi possível resolver o SecretRef gateway.auth.token para a sondagem de onboarding.",
  ...
  let localGatewayPassword =
    process.env.OPENCRAFT_GATEWAY_PASSWORD ?? process.env.CLAWDBOT_GATEWAY_PASSWORD;
  try {
    ...
    await prompter.note(
      [
        "Não foi possível resolver o SecretRef gateway.auth.password para a sondagem de onboarding.",
        error instanceof Error ? error.message : String(error),
      ].join("\n"),
      "Autenticação do gateway",
    );
  ...
  try {
    ...
    await prompter.note(
      [
        "Não foi possível resolver o SecretRef gateway.remote.token para a sondagem de onboarding.",
        error instanceof Error ? error.message : String(error),
      ].join("\n"),
      "Autenticação do gateway",
    );
