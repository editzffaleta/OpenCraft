...
describe("canvas host state dir defaults", () => {
  it("uses OPENCRAFT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("opencraft-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenCraft Canvas");
      } finally {
        await handler.close();
      }
    });
