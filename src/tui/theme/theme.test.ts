...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      OPENCRAFT_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when OPENCRAFT_THEME=light", async () => {
    const mod = await importThemeWithEnv({ OPENCRAFT_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when OPENCRAFT_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ OPENCRAFT_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats OPENCRAFT_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ OPENCRAFT_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("OPENCRAFT_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      OPENCRAFT_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ OPENCRAFT_THEME: "light" });
    const darkMod = await importThemeWithEnv({ OPENCRAFT_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.OPENCRAFT_THEME = "light";
