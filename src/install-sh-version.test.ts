...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "opencraft-install-sh-"));
  const cliPath = path.join(root, "opencraft");
...
OPENCRAFT_BIN="$FAKE_OPENCRAFT_BIN"
resolve_opencraft_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_OPENCRAFT_BIN: cliPath,
        OPENCRAFT_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("OpenCraft 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("OpenCraft dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("OpenCraft dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "opencraft-install-stdin-"));
