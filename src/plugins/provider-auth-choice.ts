...
function restoreConfiguredPrimaryModel(
  ...
  originalConfig: OpenCraftConfig,
): OpenCraftConfig {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: OpenCraftConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveOpenCraftAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
