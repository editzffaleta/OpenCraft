...
export type OpenCraftPluginConfigSchema = {
...
export type OpenCraftPluginToolContext = {
...
export type OpenCraftPluginToolFactory = (
  ctx: OpenCraftPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type OpenCraftPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type OpenCraftPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). OpenCraft uses this to expose
 ...
 *
 * Runs after OpenCraft resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common OpenCraft auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and OpenCraft wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before OpenCraft builds its generic
...
/**
 * Provider-owned stream wrapper hook after OpenCraft applies its generic
 ...
 *
 * Return `true` or `false` to override OpenCraft's built-in provider cache TTL
 ...
 *
 * Runs only after OpenCraft exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after OpenCraft loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * OpenCraft calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic OpenCraft wrappers.
   *
   ...
   *
   * Called after OpenCraft resolves the raw configured credential but before the
   ...
   * hint than OpenCraft's generic auth-store guidance.
   */
   ...
   * `errorMessage` when OpenCraft should surface a provider-specific hint for
   ...
   * merging. OpenCraft deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * OpenCraft uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `opencraft doctor`.
   *
   ...
   *
   * OpenCraft calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep OpenCraft's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type OpenCraftPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current OpenCraft configuration */
  config: OpenCraftConfig;
 ...
 */
export type OpenCraftPluginCommandDefinition = {
...
export type OpenCraftPluginHttpRouteAuth = "gateway" | "plugin";
export type OpenCraftPluginHttpRouteMatch = "exact" | "prefix";

export type OpenCraftPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type OpenCraftPluginHttpRouteParams = {
  path: string;
  handler: OpenCraftPluginHttpRouteHandler;
  auth: OpenCraftPluginHttpRouteAuth;
  match?: OpenCraftPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type OpenCraftPluginCliContext = {
  program: Command;
  config: OpenCraftConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type OpenCraftPluginCliRegistrar = (ctx: OpenCraftPluginCliContext) => void | Promise<void>;

export type OpenCraftPluginServiceContext = {
  config: OpenCraftConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type OpenCraftPluginService = {
  id: string;
  start: (ctx: OpenCraftPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: OpenCraftPluginServiceContext) => void | Promise<void>;
};

export type OpenCraftPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

export type OpenCraftPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: OpenCraftPluginConfigSchema;
  register?: (api: OpenCraftPluginApi) => void | Promise<void>;
  activate?: (api: OpenCraftPluginApi) => void | Promise<void>;
};

export type OpenCraftPluginModule =
  | OpenCraftPluginDefinition
  | ((api: OpenCraftPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

export type OpenCraftPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | OpenCraftPluginToolFactory,
    opts?: OpenCraftPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: OpenCraftPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: OpenCraftPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: OpenCraftPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: OpenCraftPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: OpenCraftPluginService) => void;
...
export type PluginFormat = "opencraft" | "bundle";
