...
import { resolveOpenCraftPackageRootSync } from "../infra/opencraft-root.js";
...
import { discoverOpenCraftPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`opencraft/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenCraftPluginDefinition;
  register?: OpenCraftPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenCraftPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenCraftPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "opencraft",
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverOpenCraftPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverOpenCraftPlugins>["candidates"][number];
  right: ReturnType<typeof discoverOpenCraftPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadOpenCraftPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadOpenCraftPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "opencraft/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into OpenCraft yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: OpenCraftPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OpenCraftPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
