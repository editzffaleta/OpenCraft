...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".opencraft", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".opencraft", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "opencraft-home");
    const roots = withEnv(
      {
        OPENCRAFT_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        OPENCRAFT_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            OPENCRAFT_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".opencraft", "extensions"),
    });
  });
});
