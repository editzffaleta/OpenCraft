...
type PluginLoadConfig = NonNullable<Parameters<typeof loadOpenCraftPlugins>[0]>["config"];
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadOpenCraftPlugins>[0], "cache" | "config">,
) {
  return loadOpenCraftPlugins({
    cache: false,
    ...options,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".opencraft", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ OPENCRAFT_STATE_DIR: stateDir, ...params.env }, () =>
    loadOpenCraftPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadOpenCraftPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadOpenCraftPlugins>;
  loadSecond: () => ReturnType<typeof loadOpenCraftPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadOpenCraftPlugins>;
  loadVariant: () => ReturnType<typeof loadOpenCraftPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  ...
  const stateDir = makeTempDir();
  const pluginDir = path.join(opencraftHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadOpenCraftPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadOpenCraftPlugins(options),
          loadVariant: () =>
            loadOpenCraftPlugins({
              ...options,
              runtimeOptions: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadOpenCraftPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadOpenCraftPlugins>) => {
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@opencraft/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opencraft/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opencraft/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opencraft/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ OPENCRAFT_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it.each([
    ...
    },
    {
      ...
      argv1: (root: string) => path.join(root, "opencraft.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
