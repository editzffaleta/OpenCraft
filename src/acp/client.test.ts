...
const createTempDir = () => tempDirs.make("opencraft-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPENCRAFT_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "opencraft",
    });

    expect(env.OPENCRAFT_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("opencraft");
  });

  it("overrides pre-existing OPENCRAFT_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPENCRAFT_SHELL: "wrong",
    });
    expect(env.OPENCRAFT_SHELL).toBe("acp-client");
  });
  ...
  it("preserves OPENCRAFT_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OPENCRAFT_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OPENCRAFT_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenCraft bridge", () => {
    ...
    expect(env.OPENCRAFT_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.OPENCRAFT_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "opencraft",
        serverArgs: ["acp"],
        defaultServerCommand: "opencraft",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("OPENCRAFT_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "opencraft", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "opencraft", "dist", "entry.js");
    const shimPath = path.join(dir, "opencraft.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\opencraft\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/opencraft-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/opencraft-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/opencraft-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
