...
const RELAY_AUTH_HEADER = "x-opencraft-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/OpenCraft-Extension-Relay",
            revision: "0",
            userAgent: "OpenCraft-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "OpenCraft/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedOpenCraftRelay({
