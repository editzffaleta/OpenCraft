...
import {
  probeAuthenticatedOpenCraftRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedOpenCraftRelay({
    baseUrl,
    relayAuthHeader: "x-opencraft-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated opencraft relay probe responses", async () => {
