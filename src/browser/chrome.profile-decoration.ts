...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opencraft-profile-decorated");
}
 ...
 */
export function decorateOpenCraftProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENCRAFT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENCRAFT_BROWSER_COLOR).toUpperCase();
