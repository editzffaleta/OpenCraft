...
export function resolveOpenCraftUserDataDir(profileName = DEFAULT_OPENCRAFT_BROWSER_PROFILE_NAME) {
...
export async function launchOpenCraftChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPENCRAFT_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOpenCraftProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 opencraft browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`opencraft browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`opencraft browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 opencraft browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOpenCraftChrome(
