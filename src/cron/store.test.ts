...
const { makeStorePath } = createCronStoreHarness({ prefix: "opencraft-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENCRAFT_HOME for tilde expansion", () => {
    vi.stubEnv("OPENCRAFT_HOME", "/srv/opencraft-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/opencraft-home", "cron", "jobs.json"));
  });
});
