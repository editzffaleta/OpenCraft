...
function resolveAgentName(cfg: OpenCraftConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenCraftConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenCraftConfig): AgentSummary[] {
