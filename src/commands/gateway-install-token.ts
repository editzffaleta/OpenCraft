...
import { readConfigFileSnapshot, writeConfigFile, type OpenCraftConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("opencraft config set gateway.auth.mode token")} or ${formatCliCommand("opencraft config set gateway.auth.mode password")}.`,
  ].join(" ");
}
