...
const SEARCH_TOOL = "https://docs.opencraft.ai/mcp.SearchOpenCraft";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.opencraft.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('opencraft docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.opencraft.ai/");
      runtime.log(`Search: ${formatCliCommand('opencraft docs "your query"')}`);
    }
    return;
  }
