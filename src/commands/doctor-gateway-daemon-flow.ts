...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.OPENCRAFT_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("opencraft gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
