...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "OPENCRAFT_GATEWAY_TOKEN",
          placeholder: "OPENCRAFT_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like OPENCRAFT_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. OpenCraft will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: OpenCraft trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.opencraft.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
