...
export function requireOpenCraftAgentDir(): string {
  const agentDir = process.env.OPENCRAFT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENCRAFT_AGENT_DIR not set");
  }
  return agentDir;
}
