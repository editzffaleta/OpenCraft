...
import { resolveOpenCraftPackageRoot } from "../infra/opencraft-root.js";
...
function resolveMode(cfg: OpenCraftConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenCraft doctor");

  const root = await resolveOpenCraftPackageRoot({
  ...
  let cfg: OpenCraftConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("opencraft configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opencraft config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opencraft setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("opencraft config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("opencraft config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("opencraft doctor --fix")}" to apply changes.`);
  }
