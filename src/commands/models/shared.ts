...
export async function loadValidConfigOrThrow(): Promise<OpenCraftConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenCraftConfig) => OpenCraftConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenCraftConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenCraftConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opencraft agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
