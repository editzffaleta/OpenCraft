...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: OpenCraftConfig;
  let lastUpdatedConfig: OpenCraftConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/opencraft/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/opencraft/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/opencraft/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: OpenCraftConfig) => OpenCraftConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/opencraft/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/opencraft/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `opencraft plugins list --json`.',
    );
  });
