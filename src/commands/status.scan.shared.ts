...
export function hasExplicitMemorySearchConfig(cfg: OpenCraftConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: OpenCraftConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: OpenCraftConfig, agentId: string) => { store: { path: string } } | null;
