...
type EnvSnapshot = {
  HOME?: string;
  OPENCRAFT_HOME?: string;
  OPENCRAFT_STATE_DIR?: string;
  OPENCRAFT_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENCRAFT_HOME: process.env.OPENCRAFT_HOME,
    OPENCRAFT_STATE_DIR: process.env.OPENCRAFT_STATE_DIR,
    OPENCRAFT_OAUTH_DIR: process.env.OPENCRAFT_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpenCraftConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpenCraftConfig) {
...
async function runStateIntegrityText(cfg: OpenCraftConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "opencraft-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPENCRAFT_HOME = tempHome;
    process.env.OPENCRAFT_STATE_DIR = path.join(tempHome, ".opencraft");
    delete process.env.OPENCRAFT_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENCRAFT_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENCRAFT_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENCRAFT_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenCraftConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints opencraft-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpenCraftConfig = {};
    ...
    expect(text).toMatch(/opencraft sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/opencraft sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /opencraft sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
