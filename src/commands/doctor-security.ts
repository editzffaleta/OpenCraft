...
import type { OpenCraftConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: OpenCraftConfig): string[] {
...
export async function noteSecurityWarnings(cfg: OpenCraftConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opencraft security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.opencraft/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("opencraft approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opencraft configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opencraft config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opencraft doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opencraft config set gateway.bind loopback")}`,
