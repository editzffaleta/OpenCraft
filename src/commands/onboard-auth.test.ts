...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENCRAFT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opencraft-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-oauth-sync-"));
    ...
    process.env.OPENCRAFT_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-oauth-nosync-"));
    process.env.OPENCRAFT_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENCRAFT_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-oauth-external-"));
    process.env.OPENCRAFT_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENCRAFT_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENCRAFT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opencraft-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
