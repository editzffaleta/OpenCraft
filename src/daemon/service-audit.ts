...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds OPENCRAFT_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `opencraft gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENCRAFT_GATEWAY_TOKEN does not match gateway.auth.token in opencraft.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.OPENCRAFT_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.OPENCRAFT_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `opencraft gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
