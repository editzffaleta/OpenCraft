...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OpenCraftConfig, policy: ChannelAccessPolicy) => OpenCraftConfig;
  resolveAllowlist: (params: { cfg: OpenCraftConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpenCraftConfig; resolved: TResolved }) => OpenCraftConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpenCraftConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpenCraftConfig }) => params.cfg);
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const applyAllowlist = vi.fn((params: { cfg: OpenCraftConfig; resolved: string[] }) => {
      calls.push("apply");
