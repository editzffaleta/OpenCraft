    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opencraft nodes status", "List known nodes with live status."],
          ["opencraft nodes pairing pending", "Show pending node pairing requests."],
          ['opencraft nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["opencraft nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.opencraft.ai/cli/nodes")}\n`,
    );
