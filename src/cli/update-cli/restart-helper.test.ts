...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'opencraft-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses OPENCRAFT_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCRAFT_PROFILE: "default",
        OPENCRAFT_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses OPENCRAFT_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        OPENCRAFT_PROFILE: "default",
        OPENCRAFT_LAUNCHD_LABEL: "com.custom.opencraft",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.opencraft'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "OpenCraft Gateway"');
      expect(content).toContain('schtasks /Run /TN "OpenCraft Gateway"');
    ...
    it("uses OPENCRAFT_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      ...
      expect(content).toContain('schtasks /End /TN "OpenCraft Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "OpenCraft Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCRAFT_PROFILE: "production",
      });
      expect(content).toContain("opencraft-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "OpenCraft Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
