    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "opencraft node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["opencraft node status", "Check node host service status."],
          ["opencraft node install", "Install the node host service."],
          ["opencraft node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.opencraft.ai/cli/node")}\n`,
    );
