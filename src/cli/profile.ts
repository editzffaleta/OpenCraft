...
export function applyCliProfileEnv(params: {
  ...
  env.OPENCRAFT_PROFILE = profile;

  const stateDir = env.OPENCRAFT_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.OPENCRAFT_STATE_DIR?.trim()) {
    env.OPENCRAFT_STATE_DIR = stateDir;
  }

  if (!env.OPENCRAFT_CONFIG_PATH?.trim()) {
    env.OPENCRAFT_CONFIG_PATH = path.join(stateDir, "opencraft.json");
  }

  if (profile === "dev" && !env.OPENCRAFT_GATEWAY_PORT?.trim()) {
    env.OPENCRAFT_GATEWAY_PORT = "19001";
  }
}
