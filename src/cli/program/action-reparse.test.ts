...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "opencraft", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "opencraft", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "opencraft", "status"]);
  });
