...
${formatHelpExamples([
  ['opencraft agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opencraft agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['opencraft agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opencraft.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opencraft.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'opencraft agents set-identity --agent main --name "OpenCraft" --emoji "🦞"',
    "Set name + emoji.",
  ],
  ...
  [
    "opencraft agents set-identity --identity-file ~/.opencraft/workspace/IDENTITY.md --agent main",
