    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opencraft memory status", "Show index and provider status."],
          ["opencraft memory status --deep", "Probe embedding provider readiness."],
          ["opencraft memory index --force", "Force a full reindex."],
          ['opencraft memory search "meeting notes"', "Quick search using positional query."],
          [
            'opencraft memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["opencraft memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.opencraft.ai/cli/memory")}\n`,
    );
