...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "opencraft config set secrets.providers.vault --provider-source file --provider-path /etc/opencraft/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("opencraft doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: OpenCraftConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as OpenCraftConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "opencraft.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.opencraft.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing opencraft.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
