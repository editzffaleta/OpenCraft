...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "opencraft";
...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: OpenCraftConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: OpenCraftConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.opencraft.ai/cli/plugins")}\n`,
    );
      ...
      if (!opts.verbose) {
        ...
        const rows = list.map((plugin) => {
          ...
          return {
            ...
            Format: plugin.format ?? "opencraft",
            Status:
              plugin.status === "loaded"
                ? theme.success("loaded")
                : plugin.status === "disabled"
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "opencraft"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: OpenCraftConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.opencraft.ai/plugin");
      lines.push("");
