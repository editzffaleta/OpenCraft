...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opencraft security audit", "Run a local security audit."],
          ["opencraft security audit --deep", "Include best-effort live Gateway probe checks."],
          ["opencraft security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "opencraft security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["opencraft security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["opencraft security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.opencraft.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("OpenCraft security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("opencraft security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("opencraft security audit --fix")}`));
        if (!fixResult) {
