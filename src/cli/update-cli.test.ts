    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opencraft-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "opencraft@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:opencraft/opencraft#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opencraft-update"));
          await updateCommand({ yes: true, tag: "github:opencraft/opencraft#main" });
        },
        expectedSpec: "github:opencraft/opencraft#main",
      },
      {
        name: "OPENCRAFT_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opencraft-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/opencraft-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("opencraft");
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            OPENCRAFT_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
