...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OpenCraftConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OpenCraftConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/opencraft-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OpenCraftConfig["bindings"]>[number];
