...
describe("ui i18n locale registry", () => {
  it("lists supported locales", () => {
    expect(SUPPORTED_LOCALES).toEqual(["pt-BR", "en", "zh-CN", "zh-TW", "de", "es"]);
  ...
  it("resolves browser locale fallbacks", () => {
    ...
    // Unknown browser locales fall back to pt-BR (primary language of OpenCraft).
    expect(resolveNavigatorLocale("fr-FR")).toBe("pt-BR");
    expect(resolveNavigatorLocale("ja-JP")).toBe("pt-BR");
    expect(resolveNavigatorLocale("")).toBe("pt-BR");
  });

  it("loads lazy locale translations from the registry", async () => {
    const en = await loadLazyLocaleTranslation("en");
    ...
    expect(getNestedTranslation(en, "common", "health")).toBe("Health");
    ...
    // pt-BR is now the default (eagerly loaded), not a lazy locale.
    expect(await loadLazyLocaleTranslation("pt-BR")).toBeNull();
  });
});
