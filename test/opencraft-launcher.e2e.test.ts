...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-launcher-"));
...
describe("opencraft launcher", () => {
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      ...
      'import "missing-opencraft-launcher-dep";\nexport {};\n',
      "utf8",
    );
    ...
    expect(result.stderr).toContain("missing-opencraft-launcher-dep");
