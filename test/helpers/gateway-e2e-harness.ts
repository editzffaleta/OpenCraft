...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opencraft-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opencraft");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENCRAFT_SKIP_PROVIDERS: "1",
          OPENCRAFT_SKIP_GMAIL_WATCHER: "1",
          OPENCRAFT_SKIP_CRON: "1",
          OPENCRAFT_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENCRAFT_SKIP_CANVAS_HOST: "1",
          OPENCRAFT_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
