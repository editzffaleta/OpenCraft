        ...
        extensions: [
          {
            ...
            packageJson: {
              ...
              opencraft: {
                install: { npmSpec: "@opencraft/googlechat" },
                releaseChecks: {
          ...
          },
          {
            ...
            packageJson: {
              ...
              opencraft: { install: { npmSpec: "@opencraft/feishu" } },
            },
          },
        ],
      }),
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            opencraft: {
              install: { npmSpec: "@opencraft/broken" },
              releaseChecks: {
  ...
  it("flags nested node_modules leaking into npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/opencraft",
      ]),
    ).toEqual(["extensions/tlon/node_modules/.bin/tlon", "node_modules/.bin/opencraft"]);
  });
});
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("opencraft-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("opencraft-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "opencraft-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 167772160 bytes (160.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "opencraft-2026.3.14.tgz" },
        { filename: "opencraft-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
