...
const OPENCRAFT_OPENSHELL_E2E = process.env.OPENCRAFT_E2E_OPENSHELL === "1";
const OPENCRAFT_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const OPENCRAFT_OPENSHELL_COMMAND =
  process.env.OPENCRAFT_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "opencraft-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPENCRAFT_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: OPENCRAFT_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(OPENCRAFT_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "opencraft-openshell-e2e-"));
      ...
      const gatewayName = `opencraft-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `opencraft-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        browser: {
          enabled: false,
          image: "opencraft-browser",
          containerPrefix: "opencraft-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: OPENCRAFT_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("opencraft-openshell-e2e");
