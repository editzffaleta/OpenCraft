...
IMAGE_NAME="${OPENCRAFT_IMAGE:-opencraft:local}"
EXTRA_MOUNTS="${OPENCRAFT_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${OPENCRAFT_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${OPENCRAFT_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${OPENCRAFT_DOCKER_SOCKET:-}"
TIMEZONE="${OPENCRAFT_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$OPENCRAFT_CONFIG_DIR/opencraft.json"
...
ensure_control_ui_allowed_origins() {
  if [[ "${OPENCRAFT_GATEWAY_BIND}" == "loopback" ]]; then
    return 0
  fi

  local allowed_origin_json
  local current_allowed_origins
  allowed_origin_json="$(printf '["http://127.0.0.1:%s"]' "$OPENCRAFT_GATEWAY_PORT")"
  current_allowed_origins="$(
    docker compose "${COMPOSE_ARGS[@]}" run --rm opencraft-cli \
...
sync_gateway_mode_and_bind() {
  ...
  docker compose "${COMPOSE_ARGS[@]}" run --rm opencraft-cli \
    config set gateway.bind "$OPENCRAFT_GATEWAY_BIND" >/dev/null
  echo "Pinned gateway.mode=local and gateway.bind=$OPENCRAFT_GATEWAY_BIND for Docker setup."
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "OPENCRAFT_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "OPENCRAFT_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
OPENCRAFT_CONFIG_DIR="${OPENCRAFT_CONFIG_DIR:-$HOME/.opencraft}"
OPENCRAFT_WORKSPACE_DIR="${OPENCRAFT_WORKSPACE_DIR:-$HOME/.opencraft/workspace}"

validate_mount_path_value "OPENCRAFT_CONFIG_DIR" "$OPENCRAFT_CONFIG_DIR"
validate_mount_path_value "OPENCRAFT_WORKSPACE_DIR" "$OPENCRAFT_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "OPENCRAFT_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "OPENCRAFT_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "OPENCRAFT_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "OPENCRAFT_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "OPENCRAFT_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "OPENCRAFT_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$OPENCRAFT_CONFIG_DIR"
mkdir -p "$OPENCRAFT_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$OPENCRAFT_CONFIG_DIR/identity"
mkdir -p "$OPENCRAFT_CONFIG_DIR/agents/main/agent"
mkdir -p "$OPENCRAFT_CONFIG_DIR/agents/main/sessions"

export OPENCRAFT_CONFIG_DIR
export OPENCRAFT_WORKSPACE_DIR
export OPENCRAFT_GATEWAY_PORT="${OPENCRAFT_GATEWAY_PORT:-18789}"
export OPENCRAFT_BRIDGE_PORT="${OPENCRAFT_BRIDGE_PORT:-18790}"
export OPENCRAFT_GATEWAY_BIND="${OPENCRAFT_GATEWAY_BIND:-lan}"
export OPENCRAFT_IMAGE="$IMAGE_NAME"
export OPENCRAFT_DOCKER_APT_PACKAGES="${OPENCRAFT_DOCKER_APT_PACKAGES:-}"
export OPENCRAFT_EXTENSIONS="${OPENCRAFT_EXTENSIONS:-}"
export OPENCRAFT_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export OPENCRAFT_HOME_VOLUME="$HOME_VOLUME_NAME"
export OPENCRAFT_ALLOW_INSECURE_PRIVATE_WS="${OPENCRAFT_ALLOW_INSECURE_PRIVATE_WS:-}"
export OPENCRAFT_SANDBOX="$SANDBOX_ENABLED"
export OPENCRAFT_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export OPENCRAFT_TZ="$TIMEZONE"
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${OPENCRAFT_CONFIG_DIR}:/home/node/.opencraft"
    gateway_workspace_mount="${OPENCRAFT_WORKSPACE_DIR}:/home/node/.opencraft/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${OPENCRAFT_INSTALL_DOCKER_CLI:-}" ]]; then
    export OPENCRAFT_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  OPENCRAFT_CONFIG_DIR \
  OPENCRAFT_WORKSPACE_DIR \
  ...
  OPENCRAFT_DOCKER_APT_PACKAGES \
  OPENCRAFT_EXTENSIONS \
  OPENCRAFT_SANDBOX \
  OPENCRAFT_DOCKER_SOCKET \
  DOCKER_GID \
  OPENCRAFT_INSTALL_DOCKER_CLI \
  OPENCRAFT_ALLOW_INSECURE_PRIVATE_WS \
  OPENCRAFT_TZ

if [[ "$IMAGE_NAME" == "opencraft:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  docker build \
    --build-arg "OPENCRAFT_DOCKER_APT_PACKAGES=${OPENCRAFT_DOCKER_APT_PACKAGES}" \
    --build-arg "OPENCRAFT_EXTENSIONS=${OPENCRAFT_EXTENSIONS}" \
    --build-arg "OPENCRAFT_INSTALL_DOCKER_CLI=${OPENCRAFT_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm opencraft-cli onboard --mode local --no-install-daemon
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm opencraft-cli channels login"
...
# --- Sandbox setup (opt-in via OPENCRAFT_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker opencraft-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg OPENCRAFT_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (OPENCRAFT_IMAGE=opencraft:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: OPENCRAFT_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    if ! docker compose "${BASE_COMPOSE_ARGS[@]}" run --rm --no-deps opencraft-cli \
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate opencraft-gateway
  fi
else
  ...
  # config alone.
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm opencraft-cli \
    config set agents.defaults.sandbox.mode "off" >/dev/null; then
